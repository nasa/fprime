...
class Scenario {
  public:
    ...
    Scenario(const char* const a_name  //!< The name of the scenario
             )
        : name(a_name), showRules(false) {
        this->setShowRules();
    }
  ...
  public:
    ...
    void reset() { this->reset_Scenario(); }

    //! Run the scenario until there are no more rules to apply
    //! \return The number of steps taken
    U32 run(State& state  //!< The system state
    ) {
        U32 numSteps = 0;
    ...
    Rule<State>* nextRule(State& state  //!< The system state
    ) {
    ...
    bool isDone() const { return this->isDone_Scenario(); }
  ...
  protected:
    ...
    virtual Rule<State>* nextRule_Scenario(State& state  //!< The system state
                                           ) = 0;
  ...
  private:
    ...
    //! Run helper function
    void runHelper(State& state,  //!< The system state
    ...
    //! Apply the next rule
    //! \return The number of steps taken
    void applyNextRule(Rule<State>& rule,  //!< The rule
                       State& state        //!< The state
    ) {
        if (this->showRules) {
            printf("[Scenario %s] Applying rule %s\n", this->name, rule.getName());
        }
        rule.apply(state);
    }
    ...
    //! Set showRules
    void setShowRules() {
        // Probe the filesystem directly; no shell needed
        FILE* const fp = fopen("show-rules", "r");
        if (fp != nullptr) {
            showRules = true;
            (void)fclose(fp);
        }
