...
U32 startLength(const U32 start, const U32 length) {
    assert(length > 0);
    // Compute the upper bound in 64 bits and clamp to avoid U32 wraparound
    const uint64_t upper = static_cast<uint64_t>(start) + length - 1;
    return lowerUpper(start, (upper > UINT32_MAX) ? UINT32_MAX : static_cast<U32>(upper));
}
