...
bool getFromFile(const char* const fileName, U32& value) {
    bool result = false;
...
bool appendToFile(const char* const fileName, const U32 seedValue) {
    bool result = false;
    FILE* fp = fopen(fileName, "a");
    if (fp != nullptr) {
        int status = fprintf(fp, "%" PRIu32 "\n", seedValue);
        result = (status > 0);
        (void)fclose(fp);
    }
    return result;
...
void seed() {
    U32 seedValue = 0;
    const bool seedValueOK = SeedValue::getFromFile("seed", seedValue);
    if (seedValueOK) {
...
U32 lowerUpper(const U32 lower, const U32 upper) {
    ...
    // Clamp the boundary case where inUnitInterval returns 1.0; casting an
    // out-of-range float to U32 is undefined behavior
    const U32 offset = (randFloat >= length) ? static_cast<U32>(length - 1) : static_cast<U32>(randFloat);
    return lower + offset;
}
