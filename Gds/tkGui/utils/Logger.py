...
# Tells the event listener whether to log raw data from the socket or not
BIN_LOGGER_FLAG = 1
#
# Two public functions for configuring the loggers.
# @todo: Move these to an independent module.
#
def connectOutputLogger(file=None, logger_name='output', logger_level=logging.INFO):
    """
    ...
    # Default logging level set to logging.INFO so use logger.info(<string output>)
    # to log stuff.  Other levels possible see documentation.
    logger.setLevel(logger_level)
    return logger


def connectDebugLogger(level=logging.INFO,file=None, stdout_enable=False,logger_name='debug'):
    """
...
def setBinLoggerFlag(flag_val):
    global BIN_LOGGER_FLAG
    BIN_LOGGER_FLAG = flag_val
