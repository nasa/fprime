...
def setup_pipelined_components(logger, config, dictionary, log_dir, tts_address, tts_port):
    """
    ...
    if __PIPELINE is None:
        ...
        pipeline.setup(config, dictionary, logging_prefix=log_dir)
        logger.info("Connecting to GDS at: {}:{}".format(tts_address, tts_port))
