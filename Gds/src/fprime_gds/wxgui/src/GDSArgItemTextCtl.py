...
class ArgItemTextCtl(wx.Panel):
    ...
    def __init__(self, parent, validator, label):
        ...
        self.m_argStaticText = wx.StaticText(
            self, wx.ID_ANY, label, wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.m_argStaticText.Wrap(-1)
        bSizer.Add(self.m_argStaticText, 0, wx.ALL, 5)

        if validator is not None:
            self.m_argWindow = wx.TextCtrl(
                self,
                wx.ID_ANY,
                wx.EmptyString,
                wx.DefaultPosition,
        ...
        try:
            ...
            try:
                ...
                wx.MessageBox(
                    "Argument: "
                    + text
                    + " is not decimal or hexidecimal. This argument must be a valid integer",
                    "Error",
                )
    ...
    def Validate(self, win):
        ...
        try:
            ...
            wx.MessageBox(
                "Argument: "
                + text
                + " is not a real value. This argument must have a real value",
                "Error",
            )
            textCtrl.SetBackgroundColour("pink")
            textCtrl.Refresh()
