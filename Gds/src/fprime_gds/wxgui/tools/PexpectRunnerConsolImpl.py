...
class PexpectRunnerImpl(PexpectRunnerConsolGUI.PexpectRunnerGUI):
    ...
    def AppendMessage(self, msg_text):
        ...
        if self.TextCtrlConsol.GetScrollPos(
            wx.VERTICAL
        ) + self.TextCtrlConsol.GetScrollThumb(
            wx.VERTICAL
        ) == self.TextCtrlConsol.GetScrollRange(
            wx.VERTICAL
        ):
            print("At bottom...")
            self.TextCtrlConsol.write(
                msg_text + "\n"
            )  # Write text with new line to consol
            self.TextCtrlConsol.SetScrollPos(
                wx.VERTICAL, self.TextCtrlConsol.GetScrollRange(wx.VERTICAL)
