...
def setup_opt_parse():
    ...
    parser.add_option("-d", "--xml_diff", dest="xml_diff_flag",
        help="Performs a diff on the specified XML files.",
        action="store_true", default=False)
        
    parser.add_option("-c", "--command_index_print", dest="command_index_print_flag",
...
def validate_xml(xml_list):
    ...
    for xml_path in xml_list:
        if os.path.isfile(xml_path):
            ...
            print("WARNING: XML {} is not a valid file. Rejecting file.".format(xml_path))
...
def tag_object_to_string(tag_obj):
    out = "<{}{}>".format(tag_obj[0] , "".join(" " + x[0] + "=" + '"'+ x[1] +'"' for x in tag_obj[1]))
...
def command_index_print(xml_list):
    for xml_path in xml_list:
        ...
        component_to_cmd_info = {} #component_to_cmd_info[componentName] = {cmdIndex:index , cmdRegIndex:index}
        ...
        for conn_info in connection_information:
            ...
            if source_comp_name not in component_to_cmd_info:
                component_to_cmd_info[source_comp_name] = {'cmdIndex':None , 'cmdRegIndex':None}
            if target_comp_name not in component_to_cmd_info:
                component_to_cmd_info[target_comp_name] = {'cmdIndex':None , 'cmdRegIndex':None}
        ...
        sorted_list.sort(key = lambda x: int(float(x[1]["cmdIndex"])) if x[1]["cmdIndex"] != None else -1 )
        
        #Print table
        print(xml_path + "\n")
        header_list = ["           Component Name          " , "Command Index" , "Command Registration Index"]
        len_list = [len(x) for x in header_list]
        
        print("| " + " | ".join(x for x in header_list) + " |")
        for comp_info in sorted_list:
            print_list = [comp_info[0] , comp_info[1]["cmdIndex"], comp_info[1]["cmdRegIndex"]]
            i = 0
            row_string = ""
