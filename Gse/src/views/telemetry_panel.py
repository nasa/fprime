...
class TelemetryPanel(object):
    ...
    def __select(self):
        ...
        """
        ch = telemetry_filter_panel.telemetryFilterDialog(self.__parent, self.__channel_names_list, self.__channels_active_list)
        if ch[0] == None: # if cancelled, keep current state
            return
        
        self.__e1.configure(text=ch[0])
        self.__channels_active_list = ch[1]
        self.__channel_names_list   = ch[3]
        #print ch
        if ch[0] != "None":
            self.clearTelmItems()
            self.initTelmItems()
    ...
    def setCell(self, row, col, value):
        ...
        """
        self.__table.model.data[row][col] = value
        self.__table.redrawTable()
    ...
    def refresh(self): 
        if self.__table.winfo_exists():
            ...
            update_rate = self.__config.get('performance', 'telem_table_update_rate')
            self.__after_id = self.__top.root().after(update_rate, self.refresh)
    ...
    def update(self, e):
        ...
        """
        msg = e.getCurrentChannelTelemetryItem()
        if msg != None:
