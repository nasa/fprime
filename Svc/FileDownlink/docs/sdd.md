...
|---|---|---|---|
| `Run` | `async input` | `Svc.Sched` | Run input port |
| `SendFile` | `guarded input` | `Svc.SendFileRequest` | Mutexed Sendfile input port |
| `FileComplete` | `output` | `[FileDownCompletePorts] Svc.SendFileComplete` | File complete output port |
| `bufferReturn` | `async input` | `Fw.BufferSend` | Buffer return input port |
| `bufferSendOut` | `output` | `Fw.BufferSend` | Buffer send output port |
...
| `FileReadError` | `warning high` | An error occurred reading a file |
| `FileSent` | `activity high` | The File Downlink component successfully sent a file |
| `DownlinkCanceled` | `activity high` | The File Downlink component canceled downlink of a file |
| `DownlinkPartialWarning` | `warning low` | The File Downlink component has detected a timeout. Downlink has been canceled. |
| `DownlinkPartialFail` | `warning high` | The File Downlink component has detected a timeout. Downlink has been canceled. |
| `SendDataFail` | `warning high` | The File Downlink component generated an error when trying to send a data packet. |
| `SendStarted` | `activity high` | The File Downlink component started a file download. |
| `DownlinkZeroSizeFile` | `warning high` | The File Downlink component stopped due to a zero-size file |
| `FilenameSourceOverflow` | `warning high` | Supplied filename has overflowed. This intentionally discards the filename to avoid cascading overflows. |
| `FilenameDestinationOverflow` | `warning high` | Supplied filename has overflowed |
| `SourceOutOfSandbox` | `warning high` | Source file path is outside the configured read sandbox |

### Telemetry

| Name | Type | Description |
|---|---|---|
| `FilesSent` | `U32` | The total number of files sent |
| `PacketsSent` | `U32` | The total number of packets sent |
| `Warnings` | `U32` | The total number of warnings |
