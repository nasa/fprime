...
void FileDownlink ::sendCancelPacket() {
    ...
    Fw::SerializeStatus status = buffer.getSerializer().serialize(Fw::ComPacketType::FW_PACKET_FILE);
...
void FileDownlink ::sendFilePacket(const Fw::FilePacket& filePacket) {
    ...
    Fw::SerializeStatus status = this->m_buffer.getSerializer().serialize(Fw::ComPacketType::FW_PACKET_FILE);
    FW_ASSERT(status == Fw::FW_SERIALIZE_OK);
