...
def test_send_systemResources_command(fprime_test_api):
    ...
    mem_total = fprime_test_api.await_telemetry(fprime_test_api.get_mnemonic('Svc.SystemResources') + "." + "MEMORY_TOTAL", start="NOW")
    mem_used = fprime_test_api.await_telemetry(fprime_test_api.get_mnemonic('Svc.SystemResources') + "." + "MEMORY_USED", start="NOW")
    nv_total = fprime_test_api.await_telemetry(fprime_test_api.get_mnemonic('Svc.SystemResources') + "." + "NON_VOLATILE_TOTAL", start="NOW")
    nv_free = fprime_test_api.await_telemetry(fprime_test_api.get_mnemonic('Svc.SystemResources') + "." + "NON_VOLATILE_FREE", start="NOW")    
    ...
    if (int(mem_total_list[3]) >= 1 and mem_total_list[4] == "KB" and
      ...
      int(nv_free_list[3]) >= 1 and nv_free_list[4] == "KB" ):
      print ("PASSED: Memory usage MEM_TOTAL, MEM_USED, NV_TOTAL, NV_FREE greater than a certain value 1K")
      print ("LIST VALUE:  ", mem_total_list[3], mem_used_list[3], nv_total_list[3], nv_free_list[3])
    else:
    ...
    cpu = fprime_test_api.await_telemetry(fprime_test_api.get_mnemonic('Svc.SystemResources') + "." + "CPU", start="NOW")    
    cpu_00 = fprime_test_api.await_telemetry(fprime_test_api.get_mnemonic('Svc.SystemResources') + "." + "CPU_00", start="NOW")    
    cpu_01 = fprime_test_api.await_telemetry(fprime_test_api.get_mnemonic('Svc.SystemResources') + "." + "CPU_01", start="NOW")    
    cpu_02 = fprime_test_api.await_telemetry(fprime_test_api.get_mnemonic('Svc.SystemResources') + "." + "CPU_02", start="NOW")    
    cpu_03 = fprime_test_api.await_telemetry(fprime_test_api.get_mnemonic('Svc.SystemResources') + "." + "CPU_03", start="NOW")    
    ...
    if (float(cpu_list[3]) >= 1 and 
       float(cpu_00_list[3]) >= 1 and
       float(cpu_01_list[3]) >= 1 and
       float(cpu_02_list[3]) >= 1 and
       float(cpu_03_list[3]) >= 1 ) :
      print ("PASSED : A number of CPUs greater than or equal to 1")  
      print ("PERCENT VALUE: ", cpu_list[3], cpu_00_list[3], cpu_01_list[3], cpu_02_list[3], cpu_03_list[3])
    else:
      print ("FAILED: cpu")
