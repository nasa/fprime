module Svc {
  ...
  passive component DpZLibCompressor {
    ...
    event ZLibInitError(err: I32, msg: string) \
      severity warning low \
      format "ZLib Error during initialization {} {}" \
      throttle 5

    @ ZLib error during deflate
    event ZLibDeflateError(err: I32, msg: string) \
      severity warning low \
    ...
    event BufferTooBigForZLib(
      in_buffer_size: FwSizeType
    ...
    event ZLibCompression(buffer_size: FwSizeType, compressed_size: FwSizeType) \
      severity diagnostic \
      format "Compressed buffer of size {} to {} bytes"

    @ ZLib compression memory usage
    event ZLibMemoryUsage(memory_used: FwSizeType, total_memory: FwSizeType) \
      severity diagnostic \
