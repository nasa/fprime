...
void FprimeDeframer ::framedIn_handler(FwIndexType portNum, Fw::Buffer& data, Fw::Buffer& context) {
    ...
    FW_ASSERT(data.getSize() >= FprimeProtocol::FrameHeader::SERIALIZED_SIZE + FprimeProtocol::FrameFooter::SERIALIZED_SIZE);

    data.setData(data.getData() + FprimeProtocol::FrameHeader::SERIALIZED_SIZE);
    data.setSize(data.getSize() - FprimeProtocol::FrameHeader::SERIALIZED_SIZE - FprimeProtocol::FrameFooter::SERIALIZED_SIZE);
