...
The passed-in `data` field (of type `Fw::Buffer`) of the `Svc.DataWithContext` input port is validated for the following conditions:
...
classDiagram
    class FprimeDeframer~PassiveComponent~ {
        + void framedIn_handler(FwIndexType portNum, Fw::Buffer& data, const CommsCfg::FrameContext& context)
    }
```
...
|---|---|---|---|
| `guarded input` | framedIn | `Svc.DataWithContext` | Receives a frame with optional context data |
| `output` | deframedOut | `Svc.DataWithContext` | Receives a frame with optional context data |
