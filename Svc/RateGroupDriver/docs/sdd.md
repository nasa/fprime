...
It takes the input `Svc::Cycle` port, then divides down the tick rate based on the divider set passed to `configure()`. Each output port may also specify an offset that phase-shifts its ticks. 
