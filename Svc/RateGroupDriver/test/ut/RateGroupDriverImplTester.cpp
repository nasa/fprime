...
namespace Svc {
    ...
    void RateGroupDriverImplTester::runSchedNominal(Svc::Divider dividers[], NATIVE_INT_TYPE numDividers) {
        ...
        for (NATIVE_INT_TYPE div = 0; div < numDividers; div++) {
            expected_rollover *= dividers[div].divisor;
        }
        ...
        for (NATIVE_INT_TYPE cycle = 0; cycle < iters; cycle++) {
            ...
            for (NATIVE_INT_TYPE div = 0; div < numDividers; div++) {
                if (cycle % dividers[div].divisor == dividers[div].offset) {
