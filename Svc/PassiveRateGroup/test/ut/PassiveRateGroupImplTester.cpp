...
PassiveRateGroupImplTester::PassiveRateGroupImplTester(Svc::PassiveRateGroupImpl& inst)
    : PassiveRateGroupGTestBase("testerbase", 100), m_impl(inst), m_causeOverrun(false), m_callOrder(0) {
    this->clearPortCalls();
}
...
void PassiveRateGroupImplTester::runNominal(NATIVE_UINT_TYPE contexts[],
    ...
    for (NATIVE_UINT_TYPE portNum = 0;
         portNum < (NATIVE_INT_TYPE)FW_NUM_ARRAY_ELEMENTS(this->m_impl.m_RateGroupMemberOut_OutputPort); portNum++) {
