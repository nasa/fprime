...
**IMPORTANT - RawTimeSource Limitation**: The `RawTimeSource` parameter in `configure()` only affects the *end* timestamp of each cycle. The *start* timestamp (`cycleStart`) comes from the cycle driver (e.g., `RateGroupDriver`, `LinuxTimer`), which constructs its `Os::RawTime` with `RAWTIME_DEFAULT`. If you configure a source different from the cycle driver's, `Os::RawTime` rejects the interval (the POSIX implementation returns `INVALID_PARAMS`) and the cycle time is reported as zero. **This feature is only correct when using `RAWTIME_DEFAULT` (the default), or when the cycle driver uses the same non-default source.** To change the clock used deployment-wide, override `config/Os/RawTimeSource.hpp` and set `RAWTIME_DEFAULT` (e.g. to `CLOCK_MONOTONIC`) rather than passing a non-default source here.
