...
namespace Svc {
  ...
  namespace ImmediateBase {
      ...
      )
    {
      ...
      ASSERT_from_seqDone_SIZE(1);
      ASSERT_from_seqDone(0U, 0U, 0U, Fw::CommandResponse(Fw::CommandResponse::EXECUTION_ERROR));
      // Assert events
      ASSERT_EVENTS_SIZE(1);
      ASSERT_EVENTS_CS_NoSequenceActive_SIZE(1);
      // Assert telemetry
      ...
      )
    {
      for (U32 i = 0; i < bound; ++i) {
        ...
        // Send status back
        this->invoke_to_cmdResponseIn(0, i, 0, Fw::CommandResponse(Fw::CommandResponse::OK));
        this->clearAndDispatch();
        if (i < numCommands - 1) {
          // Assert events
          ASSERT_EVENTS_SIZE(1);
