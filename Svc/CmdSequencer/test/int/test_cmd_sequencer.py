...
def test_seqgen(fprime_test_api):
    ...
    fprime_test_api.send_and_assert_command("cmdSeq.CS_AUTO", max_delay=1)
...
def test_send_seq(fprime_test_api):
    ...
    fprime_test_api.send_and_assert_command("cmdSeq.CS_VALIDATE",["/tmp/ref_test_seq.bin"], max_delay=1)    
    # sequence execute_2 auto 
    fprime_test_api.send_and_assert_command("cmdSeq.CS_RUN", ["/tmp/ref_test_seq.bin", "BLOCK"], max_delay=5)
    
    fprime_test_api.send_and_assert_command("cmdDisp.CMD_NO_OP_STRING",["test_string_execute_2 auto completed"], max_delay=1)

    ######    ######    ######
    ## Toggle Manual/AUTO
    fprime_test_api.send_and_assert_command("cmdSeq.CS_MANUAL", max_delay=1)
    fprime_test_api.send_and_assert_command("cmdSeq.CS_AUTO", max_delay=1)
    ...
    fprime_test_api.send_and_assert_command("cmdSeq.CS_RUN", ["/tmp/ref_test_seq.bin", "NO_BLOCK"], max_delay=5)
    
    # sequence execute_5 manually Command 0 in sequence
    fprime_test_api.send_and_assert_command("cmdSeq.CS_START", max_delay=1)    

    fprime_test_api.send_and_assert_command("cmdSeq.CS_CANCEL", max_delay=1)        

    fprime_test_api.send_and_assert_command("cmdDisp.CMD_NO_OP_STRING",["manually START/CANCEL after cmd 0"], max_delay=1)
    ...
    #fprime_test_api.send_and_assert_command("cmdSeq.CS_START", max_delay=1)
    fprime_test_api.send_command("cmdSeq.CS_START")

    # Load Sequence but not execute is current SEQ manual (will load sequence only)    
    fprime_test_api.send_and_assert_command("cmdSeq.CS_RUN", ["/tmp/ref_test_seq_wait.bin", "NO_BLOCK"], max_delay=5)
    
    # sequence manually mode start  Command 0 in sequence    
    fprime_test_api.send_and_assert_command("cmdSeq.CS_START", max_delay=1)
    fprime_test_api.send_and_assert_command("cmdDisp.CMD_NO_OP_STRING",["manually START command 0"], max_delay=1)


    # sequence manually mode step  Next Command 1  in sequence  (Only manual mode)          
    fprime_test_api.send_and_assert_command("cmdSeq.CS_STEP", max_delay=3)        
    fprime_test_api.send_and_assert_command("cmdDisp.CMD_NO_OP_STRING",["manually START/STEP cmd 1 "], max_delay=3)
    
    time.sleep(10)    
    fprime_test_api.send_and_assert_command("cmdSeq.CS_STEP", max_delay=1)
    time.sleep(130)    
    fprime_test_api.send_and_assert_command("cmdSeq.CS_STEP", max_delay=3)    
