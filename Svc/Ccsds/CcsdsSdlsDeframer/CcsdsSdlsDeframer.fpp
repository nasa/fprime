    ...
    module Ccsds {
        ...
        passive component CcsdsSdlsDeframer {
            ...
            event InsufficientLength() \
                ...
                format "Failed to deframe security association index: insufficient length"

            @ Event raised when the decryption helper fails
            event DecryptionFailed(status: Svc.Ccsds.SdlsStatus) \
                severity warning high \
                format "Failed to decrypt frame: {}"
