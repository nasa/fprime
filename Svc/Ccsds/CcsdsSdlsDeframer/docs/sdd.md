...
| SVC-CCSDS-SDLS-DEFRAMER-005 | Upon a non-SUCCESS status from the decryption helper, the CcsdsSdlsDeframer shall emit the `DecryptionFailed` WARNING_HI event and notify `errorNotify` with `SDLS_DECRYPTION_FAILURE`. The failing decryptor returns the buffer via `bufferReturnIn`; the deframer shall not return it directly. | Decryption failures must be visible to the system; buffer ownership returns through the decryption helper's callback. | Unit test |
