...
- **Rejection causes and length bound.** Four conditions produce the same `RxDestPathRejected` event and `faultFileOpen` increment: `rx_dir` itself cannot be resolved, the received path cannot be resolved, the resolved path lies outside `rx_dir`, or the resolved path is longer than `MaxFilePathSize`. Because the stored path is `rx_dir` joined with the received name, the longest accepted name is roughly `MaxFilePathSize` minus the length of the resolved `rx_dir`; size `rx_dir` accordingly when filenames approach the PDU limit.
...
| RxDestPathRejected | warning high | RX transaction rejected a Metadata PDU destination path that does not resolve inside the channel's `rx_dir` (see Security Considerations) |
