...
|---|---|---|
| `BuffersExhausted` | `warning low` |  |
| `InvalidChannel` | `warning low` |  |
| `TxFileQueued` | `activity low` |  |
| `UnsupportedSendFileArguments` | `warning low` |  |
| `SendFileInitiateFail` | `warning low` |  |
| `PlaybackInitiated` | `activity low` |  |
| `PollDirInitiated` | `activity low` |  |
| `PollDirStopped` | `activity low` |  |
| `PollDirBusy` | `warning low` |  |
| `PollDirNotActive` | `warning low` |  |
| `SetFlowState` | `activity low` |  |
| `InvalidChannelPoll` | `warning low` |  |
| `FailKeepFileMove` | `warning low` |  |
| `FailPollFileMove` | `warning low` |  |
| `FailPduHeaderDeserialization` | `warning low` |  |
| `FailPduSerialization` | `warning low` |  |
| `FailMetadataPduDeserialization` | `warning low` |  |
| `FailFileDataPduDeserialization` | `warning low` |  |
| `FailEofPduDeserialization` | `warning low` |  |
| `FailAckPduDeserialization` | `warning low` |  |
| `FailFinPduDeserialization` | `warning low` |  |
| `FailNakPduDeserialization` | `warning low` |  |
| `RxAckLimitReached` | `warning low` |  |
| `RxTempFileCreated` | `activity low` |  |
| `RxFileCreateFailed` | `warning low` |  |
| `RxCrcMismatch` | `warning low` |  |
| `RxNakLimitReached` | `warning low` |  |
| `RxSeekFailed` | `warning low` |  |
| `RxWriteFailed` | `warning low` |  |
| `RxFileDataOutOfBounds` | `warning low` |  |
| `RxFileSizeMismatch` | `warning low` |  |
| `RxEofCancelReceived` | `activity high` |  |
| `RxEofWithError` | `warning low` |  |
| `RxSeekCrcFailed` | `warning low` |  |
| `RxReadCrcFailed` | `warning low` |  |
| `RxEofMdSizeMismatch` | `warning low` |  |
| `RxFileRenameFailed` | `warning low` |  |
| `RxFileReopenFailed` | `warning low` |  |
| `RxInactivityTimeout` | `warning low` |  |
| `RxInvalidDirectiveCode` | `warning low` |  |
| `TxAckLimitReached` | `warning low` |  |
| `TxInactivityTimeout` | `warning low` |  |
| `TxZeroLengthFile` | `warning low` |  |
| `TxFileOpenFailed` | `warning low` |  |
| `TxFileSeekFailed` | `warning low` |  |
| `TxSendMetadataFailed` | `warning low` |  |
| `TxEarlyFinReceived` | `warning low` |  |
| `TxInvalidNakPdu` | `warning low` |  |
| `TxInvalidSegmentRequests` | `warning low` |  |
| `TxNonFileDirectivePduReceived` | `warning low` |  |
| `TxInvalidDirectiveCode` | `warning low` |  |
| `TxFileTransferStarted` | `activity high` |  |
| `TxFileTransferCompleted` | `activity high` |  |
| `TxFileTransferFailed` | `warning low` |  |
| `RxFileTransferCompleted` | `activity high` |  |
| `RxFileTransferFailed` | `warning low` |  |
| `MetadataReceived` | `activity low` |  |
| `FileDataSegmentMetadata` | `warning low` |  |
| `ChunklistUnavailable` | `warning low` |  |
| `UnhandledPduInIdleState` | `warning low` |  |
| `RxTransactionLimitReached` | `warning low` |  |
| `InvalidDestinationEid` | `warning low` |  |
| `MaxTxTransactionsReached` | `warning low` |  |
| `PlaybackDirOpenFailed` | `warning low` |  |
| `PlaybackDirSlotUnavailable` | `warning low` |  |
| `DanglingFileHandleClosed` | `warning low` |  |
| `PlaybackDirReadFailed` | `warning low` |  |
| `ResetFreedTransaction` | `diagnostic` |  |
| `FileRemoveFailed` | `warning low` |  |
| `TransactionSuspended` | `activity low` | Transaction was suspended |
| `TransactionResumed` | `activity low` | Transaction was resumed |
| `TransactionCanceled` | `activity high` | Transaction canceled |
| `TransactionAbandoned` | `activity high` | Transaction abandoned |
| `TransactionNotFound` | `warning low` | Transaction not found for command |
| `ResetCounters` | `activity high` |  |
| `TxLateFinAcked` | `diagnostic` | A FIN arrived for a downlink transaction we sourced that has already completed and been recycled; acknowledged statelessly per CFDP so the peer can close out. |

### Parameters

| Name | Type | Description |
|---|---|---|
| `LocalEid` | `Cfdp.EntityId` | CFDP ID to denote the current node when sending PDUs |
| `OutgoingFileChunkSize` | `U32` | Maximum number of bytes to put into a file PDU |
| `RxCrcCalcBytesPerCycle` | `U32` | The maximum number of received bytes to calculate a CRC for in a single scheduler cycle |
| `PostInactivitySendRetries` | `U8` | Extra scheduler cycles a pending terminal send (EOF or FIN-ACK) is retried after the inactivity timer fires before the transaction is recycled regardless |
| `FileInDefaultChannel` | `U8` | Default CFDP channel for fileIn port-initiated file transfers |
| `FileInDefaultDestEntityId` | `Cfdp.EntityId` | Default destination entity ID for fileIn port-initiated file transfers |
| `FileInDefaultClass` | `Cfdp.Class` | Default CFDP class for fileIn port-initiated file transfers |
| `FileInDefaultKeep` | `Cfdp.Keep` | Default file retention policy for fileIn port-initiated file transfers |
| `FileInDefaultPriority` | `U8` | Default priority for fileIn port-initiated file transfers |
| `ChannelConfig` | `Cfdp.ChannelArrayParams` | Parameter configuration for an array CFDP channels |
