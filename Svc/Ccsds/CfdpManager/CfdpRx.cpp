...
//
// Handles all CFDP m_engine functionality specific to RX transactions.
// 
// ======================================================================
//
...
{
    txn->m_engine->setTxnStatus(txn, txn_stat);
...
{
    if ((txn->m_state_data.receive.sub_state == CF_RxSubState_CLOSEOUT_SYNC) ||
        (txn->m_state_data.receive.r2.eof_cc != CF_CFDP_ConditionCode_NO_ERROR) ||
        CF_TxnStatus_IsError(txn->m_history->txn_stat) || txn->m_flags.com.canceled)
    {
...
CfdpStatus::T CF_CFDP_R_CheckCrc(CfdpTransaction *txn, U32 expected_crc)
{
    ...
    if (crc_result != expected_crc)
    {
        ...
        //                   (txn->m_state == CF_TxnState_R2), (unsigned long)txn->m_history->src_eid,
        //                   (unsigned long)txn->m_history->seq_num, (unsigned long)crc_result,
...
void CF_CFDP_R2_Complete(CfdpTransaction *txn, bool ok_to_send_nak)
{
    U32 ret;
    bool send_nak = false;
    bool send_fin = false;
    ...
    if (!CF_TxnStatus_IsError(txn->m_history->txn_stat))
    {
        ...
        else
        {
            ...
            ret = CF_ChunkList_ComputeGaps(&txn->m_chunks->chunks, 1, txn->m_fsize, 0, NULL, NULL);
            ...
            else if (txn->m_flags.rx.eof_recv)
            {
        ...
        if (send_nak && ok_to_send_nak)
        {
            ...
            ++txn->m_state_data.receive.r2.acknak_count;

            /* Check limit and handle if needed */
            nack_limit = txn->m_cfdpManager->getNackLimitParam(txn->m_chan_num);
            if (txn->m_state_data.receive.r2.acknak_count >= nack_limit)
            {
                // CFE_EVS_SendEvent(CF_CFDP_R_NAK_LIMIT_ERR_EID, CFE_EVS_EventType_ERROR,
                //                   "CF R%d(%lu:%lu): NAK limited reach", (txn->m_state == CF_TxnState_R2),
                //                   (unsigned long)txn->m_history->src_eid, (unsigned long)txn->m_history->seq_num);
                send_fin = true;
                // ++CF_AppData.hk.Payload.channel_hk[txn->m_chan_num].counters.fault.nak_limit;
                /* don't use CF_CFDP_R2_SetFinTxnStatus because many places in this function set send_fin */
                txn->m_engine->setTxnStatus(txn, CF_TxnStatus_NAK_LIMIT_REACHED);
                txn->m_state_data.receive.r2.acknak_count = 0; /* reset for fin/ack */
            }
            else
            {
                txn->m_flags.rx.send_nak = true;
            }
        }

        if (send_fin)
        {
            txn->m_flags.rx.complete = true; /* latch completeness, since send_fin is cleared later */
        ...
        txn->m_state_data.receive.sub_state = CF_RxSubState_FILEDATA;
    }
}
...
CfdpStatus::T CF_CFDP_R_ProcessFd(CfdpTransaction *txn, CF_Logical_PduBuffer_t *ph)
{
    ...
    if (txn->m_state_data.receive.cached_pos != pdu->offset)
    {
        status = txn->m_fd.seek(pdu->offset, Os::File::SeekType::ABSOLUTE);
        if (status != Os::File::OP_OK)
        {
            // CFE_EVS_SendEvent(CF_CFDP_R_SEEK_FD_ERR_EID, CFE_EVS_EventType_ERROR,
            //                   "CF R%d(%lu:%lu): failed to seek offset %ld, got %ld", (txn->m_state == CF_TxnState_R2),
            //                   (unsigned long)txn->m_history->src_eid, (unsigned long)txn->m_history->seq_num,
            //                   (long)pdu->offset, (long)fret);
            txn->m_engine->setTxnStatus(txn, CF_TxnStatus_FILE_SIZE_ERROR);
    ...
    if (ret != CfdpStatus::ERROR)
    {
        ...
        status = txn->m_fd.write(pdu->data_ptr, write_size, Os::File::WaitType::WAIT);
        if (status != Os::File::OP_OK)
        {
            // CFE_EVS_SendEvent(CF_CFDP_R_WRITE_ERR_EID, CFE_EVS_EventType_ERROR,
            //                   "CF R%d(%lu:%lu): OS_write expected %ld, got %ld", (txn->m_state == CF_TxnState_R2),
            //                   (unsigned long)txn->m_history->src_eid, (unsigned long)txn->m_history->seq_num,
            //                   (long)pdu->data_len, (long)fret);
            txn->m_engine->setTxnStatus(txn, CF_TxnStatus_FILESTORE_REJECTION);
            // ++CF_AppData.hk.Payload.channel_hk[txn->m_chan_num].counters.fault.file_write;
            ret = CfdpStatus::ERROR; /* connection will reset in caller */
        }
        else
        {
            txn->m_state_data.receive.cached_pos = static_cast<U32>(pdu->data_len) + pdu->offset;
            // CF_AppData.hk.Payload.channel_hk[txn->m_chan_num].counters.recv.file_data_bytes += pdu->data_len;
        }
    }
...
CfdpStatus::T CF_CFDP_R_SubstateRecvEof(CfdpTransaction *txn, CF_Logical_PduBuffer_t *ph)
{
    ...
    if (!txn->m_engine->recvEof(txn, ph))
    {
        ...
        if (txn->m_flags.rx.md_recv && (eof->size != txn->m_fsize))
        {
            ...
            //                   (txn->m_state == CF_TxnState_R2), (unsigned long)txn->m_history->src_eid,
            //                   (unsigned long)txn->m_history->seq_num, (unsigned long)eof->size,
            //                   (unsigned long)txn->m_fsize);
            // ++CF_AppData.hk.Payload.channel_hk[txn->m_chan_num].counters.fault.file_size_mismatch;
...
void CF_CFDP_R1_SubstateRecvEof(CfdpTransaction *txn, CF_Logical_PduBuffer_t *ph)
{
    ...
    if (ret == CfdpStatus::SUCCESS)
    {
        ...
        if (CF_CFDP_R_CheckCrc(txn, crc) == CfdpStatus::SUCCESS)
        {
            ...
            txn->m_keep = CfdpKeep::KEEP; /* save the file */
        }
...
void CF_CFDP_R2_SubstateRecvEof(CfdpTransaction *txn, CF_Logical_PduBuffer_t *ph)
{
    ...
    if (!txn->m_flags.rx.eof_recv)
    {
        ...
        if (ret == CfdpStatus::SUCCESS)
        {
            ...
            txn->m_state_data.receive.r2.eof_crc  = eof->crc;
            txn->m_state_data.receive.r2.eof_size = eof->size;

            /* always ACK the EOF, even if we're not done */
            txn->m_state_data.receive.r2.eof_cc = eof->cc;
            txn->m_flags.rx.send_eof_ack        = true; /* defer sending ACK to tick handling */

            /* only check for complete if EOF with no errors */
            if (txn->m_state_data.receive.r2.eof_cc == CF_CFDP_ConditionCode_NO_ERROR)
            {
            ...
            else
            {
                ...
                txn->m_engine->setTxnStatus(txn, static_cast<CF_TxnStatus_t>(txn->m_state_data.receive.r2.eof_cc));
...
void CF_CFDP_R1_SubstateRecvFileData(CfdpTransaction *txn, CF_Logical_PduBuffer_t *ph)
{
    ...
    /* got file data PDU? */
    ret = txn->m_engine->recvFd(txn, ph);
    if (ret == CfdpStatus::SUCCESS)
    {
    ...
    {
        /* class 1 digests CRC */
        txn->m_crc.update(ph->int_header.fd.data_ptr, ph->int_header.fd.offset,
...
void CF_CFDP_R2_SubstateRecvFileData(CfdpTransaction *txn, CF_Logical_PduBuffer_t *ph)
{
    ...
    if (txn->m_state_data.receive.r2.rx_crc_calc_bytes > 0)
    {
    ...
    if (ret == CfdpStatus::SUCCESS)
    {
        ...
        CF_ChunkListAdd(&txn->m_chunks->chunks, fd->offset, static_cast<CF_ChunkSize_t>(fd->data_len));
        ...
        if (!txn->m_flags.rx.complete)
        {
            txn->m_engine->armAckTimer(txn); /* re-arm ACK timer, since we got data */
        }

        txn->m_state_data.receive.r2.acknak_count = 0;
    }
    else
    {
        /* Reset transaction on failure */
...
void CF_CFDP_R2_GapCompute(const CF_ChunkList_t *chunks, const CF_Chunk_t *chunk, void *opaque)
{
    ...
    /* it seems that scope in the old m_engine is not used the way I read it in the spec, so
...
CfdpStatus::T CF_CFDP_R_SubstateSendNak(CfdpTransaction *txn)
{
    CF_Logical_PduBuffer_t *ph =
        txn->m_engine->constructPduHeader(txn, CF_CFDP_FileDirective_NAK, txn->m_history->peer_eid,
                                   txn->m_cfdpManager->getLocalEidParam(), 1, txn->m_history->seq_num, true);
    CF_Logical_PduNak_t *nak;
    CfdpStatus::T sret;
    U32 cret;
    ...
    if (ph)
    {
        ...
        if (txn->m_flags.rx.md_recv)
        {
            ...
            nak->scope_start = 0;
            cret             = CF_ChunkList_ComputeGaps(&txn->m_chunks->chunks,
                                            (txn->m_chunks->chunks.count < txn->m_chunks->chunks.max_chunks)
                                                            ? txn->m_chunks->chunks.max_chunks
                                                            : (txn->m_chunks->chunks.max_chunks - 1),
                                            txn->m_fsize, 0, CF_CFDP_R2_GapCompute, &args);

            if (!cret)
            {
                /* no gaps left, so go ahead and check for completion */
                txn->m_flags.rx.complete = true; /* we know md was received, and there's no gaps -- it's complete */
                ret                    = CfdpStatus::SUCCESS;
            }
            else
            {
                ...
                nak->scope_end = 0;
                sret = txn->m_engine->sendNak(txn, ph);
                txn->m_flags.rx.fd_nak_sent = true; /* latch that at least one NAK has been sent requesting filedata */
                ...
                {
                    // CF_AppData.hk.Payload.channel_hk[txn->m_chan_num].counters.sent.nak_segment_requests += cret;
                    ret = CfdpStatus::SUCCESS;
                }
            }
        }
        else
        {
            ...
            //                   "CF R%d(%lu:%lu): requesting MD", (txn->m_state == CF_TxnState_R2),
            //                   (unsigned long)txn->m_history->src_eid, (unsigned long)txn->m_history->seq_num);
...
void CF_CFDP_R_Init(CfdpTransaction *txn)
{
    ...
    if (txn->m_state == CF_TxnState_R2)
    {
        if (!txn->m_flags.rx.md_recv)
        {
            tmpDir = txn->m_cfdpManager->getTmpDirParam();
            ...
            //                   (txn->m_state == CF_TxnState_R2), (unsigned long)txn->m_history->src_eid,
            //                   (unsigned long)txn->m_history->seq_num, txn->m_history->fnames.dst_filename);
        }

        txn->m_engine->armAckTimer(txn);
    }

    status = txn->m_fd.open(txn->m_history->fnames.dst_filename.toChar(), Os::File::OPEN_CREATE, Os::File::OVERWRITE);
    if (status != Os::File::OP_OK)
    {
        ...
        //                   (txn->m_state == CF_TxnState_R2), (unsigned long)txn->m_history->src_eid,
        //                   (unsigned long)txn->m_history->seq_num, txn->m_history->fnames.dst_filename, (long)ret);
...
CfdpStatus::T CF_CFDP_R2_CalcCrcChunk(CfdpTransaction *txn)
{
    ...
    if (txn->m_state_data.receive.r2.rx_crc_calc_bytes == 0)
    {
        ...
        txn->m_state_data.receive.cached_pos = 0;
    }

    rx_crc_calc_bytes_per_wakeup = txn->m_cfdpManager->getRxCrcCalcBytesPerWakeupParam();

    while ((count_bytes < rx_crc_calc_bytes_per_wakeup) &&
           (txn->m_state_data.receive.r2.rx_crc_calc_bytes < txn->m_fsize))
    {
        want_offs_size = txn->m_state_data.receive.r2.rx_crc_calc_bytes + sizeof(buf);

        if (want_offs_size > txn->m_fsize)
        {
            read_size = txn->m_fsize - txn->m_state_data.receive.r2.rx_crc_calc_bytes;
        }
        else
        {
        ...
        if (txn->m_state_data.receive.cached_pos != txn->m_state_data.receive.r2.rx_crc_calc_bytes)
        {
            fileStatus = txn->m_fd.seek(txn->m_state_data.receive.r2.rx_crc_calc_bytes, Os::File::SeekType::ABSOLUTE);
            if (fileStatus != Os::File::OP_OK)
            {
                // CFE_EVS_SendEvent(CF_CFDP_R_SEEK_CRC_ERR_EID, CFE_EVS_EventType_ERROR,
                //                   "CF R%d(%lu:%lu): failed to seek offset %lu, got %ld", (txn->m_state == CF_TxnState_R2),
                //                   (unsigned long)txn->m_history->src_eid, (unsigned long)txn->m_history->seq_num,
                //                   (unsigned long)txn->m_state_data.receive.r2.rx_crc_calc_bytes, (long)fret);
                // txn->m_engine->setTxnStatus(txn, CF_TxnStatus_FILE_SIZE_ERROR);
                // ++CF_AppData.hk.Payload.channel_hk[txn->m_chan_num].counters.fault.file_seek;
                success = false;
                break;
        ...
        fileStatus = txn->m_fd.read(buf, read_size, Os::File::WaitType::WAIT);
        if (fileStatus != Os::File::OP_OK)
        {
            // CFE_EVS_SendEvent(CF_CFDP_R_READ_ERR_EID, CFE_EVS_EventType_ERROR,
            //                   "CF R%d(%lu:%lu): failed to read file expected %lu, got %ld",
            //                   (txn->m_state == CF_TxnState_R2), (unsigned long)txn->m_history->src_eid,
            //                   (unsigned long)txn->m_history->seq_num, (unsigned long)read_size, (long)fret);
            txn->m_engine->setTxnStatus(txn, CF_TxnStatus_FILE_SIZE_ERROR);
        ...
        txn->m_crc.update(buf, txn->m_state_data.receive.r2.rx_crc_calc_bytes, static_cast<U32>(read_size));
        txn->m_state_data.receive.r2.rx_crc_calc_bytes += static_cast<U32>(read_size);
        txn->m_state_data.receive.cached_pos = txn->m_state_data.receive.r2.rx_crc_calc_bytes;
        count_bytes += read_size;
    }

    if (success && txn->m_state_data.receive.r2.rx_crc_calc_bytes == txn->m_fsize)
    {
        /* all bytes calculated, so now check */
        if (CF_CFDP_R_CheckCrc(txn, txn->m_state_data.receive.r2.eof_crc) == CfdpStatus::SUCCESS)
        {
            /* CRC matched! We are happy */
            txn->m_keep = CfdpKeep::KEEP; /* save the file */

            /* set FIN PDU status */
            txn->m_state_data.receive.r2.dc = CF_CFDP_FinDeliveryCode_COMPLETE;
            txn->m_state_data.receive.r2.fs = CF_CFDP_FinFileStatus_RETAINED;
        }
        else
        {
...
CfdpStatus::T CF_CFDP_R2_SubstateSendFin(CfdpTransaction *txn)
{
    ...
    if (!CF_TxnStatus_IsError(txn->m_history->txn_stat) && !txn->m_flags.com.crc_calc)
    {
    ...
    if (ret != CfdpStatus::ERROR)
    {
        sret = txn->m_engine->sendFin(txn, txn->m_state_data.receive.r2.dc, txn->m_state_data.receive.r2.fs,
                               CF_TxnStatus_To_ConditionCode(txn->m_history->txn_stat));
        /* CF_CFDP_SendFin does not return SEND_PDU_ERROR */
        FW_ASSERT(sret != CfdpStatus::SEND_PDU_ERROR); 
        txn->m_state_data.receive.sub_state =
...
{
    if (!txn->m_engine->recvAck(txn, ph))
    {
...
void CF_CFDP_R2_RecvMd(CfdpTransaction *txn, CF_Logical_PduBuffer_t *ph)
{
    ...
    if (!txn->m_flags.rx.md_recv)
    {
        /* NOTE: txn->m_flags.rx.md_recv always 1 in R1, so this is R2 only */
         ...
         * the md PDU */
        fname = txn->m_history->fnames.dst_filename;

        status = txn->m_engine->recvMd(txn, ph);
        if (status == CfdpStatus::SUCCESS)
        {
            ...
            if (txn->m_flags.rx.eof_recv)
            {
                ...
                if (txn->m_state_data.receive.r2.eof_size != txn->m_fsize)
                {
                    // CFE_EVS_SendEvent(CF_CFDP_R_EOF_MD_SIZE_ERR_EID, CFE_EVS_EventType_ERROR,
                    //                   "CF R%d(%lu:%lu): EOF/md size mismatch md: %lu, EOF: %lu",
                    //                   (txn->m_state == CF_TxnState_R2), (unsigned long)txn->m_history->src_eid,
                    //                   (unsigned long)txn->m_history->seq_num, (unsigned long)txn->m_fsize,
                    //                   (unsigned long)txn->m_state_data.receive.r2.eof_size);
                    // ++CF_AppData.hk.Payload.channel_hk[txn->m_chan_num].counters.fault.file_size_mismatch;
                    CF_CFDP_R2_SetFinTxnStatus(txn, CF_TxnStatus_FILE_SIZE_ERROR);
                    success = false;
                }
            ...
            if (success)
            {
                ...
                if (fileSysStatus != Os::FileSystem::OP_OK)
                {
                    ...
                    //                   (txn->m_state == CF_TxnState_R2), (unsigned long)txn->m_history->src_eid,
                    //                   (unsigned long)txn->m_history->seq_num, (long)status);
                    // txn->m_fd = OS_OBJECT_ID_UNDEFINED;
                    CF_CFDP_R2_SetFinTxnStatus(txn, CF_TxnStatus_FILESTORE_REJECTION);
                    // ++CF_AppData.hk.Payload.channel_hk[txn->m_chan_num].counters.fault.file_rename;
                    success = false;
                }
                else
                {
                    ...
                    fileStatus = txn->m_fd.open(txn->m_history->fnames.dst_filename.toChar(), Os::File::OPEN_WRITE);
                    if (fileStatus != Os::File::OP_OK)
                    {
                        // CFE_EVS_SendEvent(CF_CFDP_R_OPEN_ERR_EID, CFE_EVS_EventType_ERROR,
                        //                   "CF R%d(%lu:%lu): failed to open renamed file in R2, error=%ld",
                        //                   (txn->m_state == CF_TxnState_R2), (unsigned long)txn->m_history->src_eid,
                        //                   (unsigned long)txn->m_history->seq_num, (long)ret);
                        CF_CFDP_R2_SetFinTxnStatus(txn, CF_TxnStatus_FILESTORE_REJECTION);
                        // ++CF_AppData.hk.Payload.channel_hk[txn->m_chan_num].counters.fault.file_open;
                        // txn->m_fd = OS_OBJECT_ID_UNDEFINED; /* just in case */
                        success = false;
                ...
                if (success)
                {
                    txn->m_state_data.receive.cached_pos      = 0; /* reset psn due to open */
                    txn->m_flags.rx.md_recv                   = true;
                    txn->m_state_data.receive.r2.acknak_count = 0; /* in case part of NAK */
...
void CF_CFDP_R1_Recv(CfdpTransaction *txn, CF_Logical_PduBuffer_t *ph)
{
    ...
    txn->rDispatchRecv(ph, &substate_fns, CF_CFDP_R1_SubstateRecvFileData);
}
...
void CF_CFDP_R_Cancel(CfdpTransaction *txn)
{
    ...
    if ((txn->m_state == CF_TxnState_R2) && (txn->m_state_data.receive.sub_state < CF_RxSubState_CLOSEOUT_SYNC))
    {
        txn->m_flags.rx.send_fin = true;
    }
    else
...
void CF_CFDP_R_SendInactivityEvent(CfdpTransaction *txn)
{
    ...
    //                   "CF R%d(%lu:%lu): inactivity timer expired", (txn->m_state == CF_TxnState_R2),
    //                   (unsigned long)txn->m_history->src_eid, (unsigned long)txn->m_history->seq_num);
...
void CF_CFDP_R_AckTimerTick(CfdpTransaction *txn)
{
    ...
    if (txn->m_state != CF_TxnState_R2 || !txn->m_flags.com.ack_timer_armed)
    {
        /* nothing to do */
        return;
    }
    ...
    else
    {
        ...
        else if (txn->m_state_data.receive.sub_state == CF_RxSubState_CLOSEOUT_SYNC)
        {
            ...
            if (txn->m_state_data.receive.r2.acknak_count >= ack_limit)
            {
                // CFE_EVS_SendEvent(CF_CFDP_R_ACK_LIMIT_ERR_EID, CFE_EVS_EventType_ERROR,
                //                   "CF R2(%lu:%lu): ACK limit reached, no fin-ack", (unsigned long)txn->m_history->src_eid,
                //                   (unsigned long)txn->m_history->seq_num);
                txn->m_engine->setTxnStatus(txn, CF_TxnStatus_ACK_LIMIT_NO_FIN);
...
void CF_CFDP_R_Tick(CfdpTransaction *txn, int *cont /* unused */)
{
    ...
    if (!txn->m_flags.com.inactivity_fired)
    {
        ...
        else
        {
            ...
            if (txn->m_state != CF_TxnState_HOLD)
            {
    ...
    {
        sret = txn->m_engine->sendAck(txn, CF_CFDP_AckTxnStatus_ACTIVE, CF_CFDP_FileDirective_EOF,
                               static_cast<CF_CFDP_ConditionCode_t>(txn->m_state_data.receive.r2.eof_cc),
        ...
        {
            txn->m_flags.rx.send_eof_ack = false;
        }
    }
    else if (txn->m_flags.rx.send_nak)
    {
        if (!CF_CFDP_R_SubstateSendNak(txn))
        {
            txn->m_flags.rx.send_nak = false; /* will re-enter on error */
        }
    }
    else if (txn->m_flags.rx.send_fin)
    {
        if (!CF_CFDP_R2_SubstateSendFin(txn))
        {
            txn->m_flags.rx.send_fin = false; /* will re-enter on error */
        }
    }
    else
    {
