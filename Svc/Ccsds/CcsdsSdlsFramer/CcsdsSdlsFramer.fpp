module Svc {
  module Ccsds {
    ...
    passive component CcsdsSdlsFramer {
      ...
      event EncryptionFailed(status: Svc.Ccsds.SdlsStatus) \
        severity warning high \
        format "Failed to encrypt frame: {}"

      @ Event raised when the allocated frame buffer is too small for the security association index and encrypted data
      event BufferAllocationFailed(allocationSize: FwSizeType) \
        severity warning high \
        format "Failed to allocate frame buffer of size {}"
