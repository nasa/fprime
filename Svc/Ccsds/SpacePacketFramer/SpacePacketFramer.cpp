...
void SpacePacketFramer ::dataIn_handler(FwIndexType portNum, Fw::Buffer& data, const ComCfg::FrameContext& context) {
    ...
    FW_ASSERT(bodySize <= std::numeric_limits<U16>::max(), static_cast<FwAssertArgType>(bodySize));
    U16 packetDataLength = static_cast<U16>(bodySize - 1);  // Standard specifies length is number of bytes minus 1
