...
* Transfer Frame Data Field is driven by the per-instance frame size passed to `configure` at runtime (bounded by the compile-time `ComCfg::AosMaxFrameFixedSize`)
