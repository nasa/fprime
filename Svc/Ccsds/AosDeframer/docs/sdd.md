...
EPP Packet Length fields include the complete packet header, as specified in CCSDS 133.1-B-3 section 4.1.2.8.2. The deframer uses this total directly for allocation, reassembly, and advancement to the next packet. Non-idle packets must declare a representable length larger than their header; absent, shorter, and header-only lengths are rejected without allocating or emitting a packet. Idle packets are discarded. Oversized allocation requests are handled by the allocation-failure path (`SpanningPacketAllocFailed`). A rejected EPP length ends packet extraction for the current frame, so any packets following it in the data zone are discarded without a separate event; extraction resumes from the next frame's First Header Pointer.

This header-inclusive interpretation corrects the previous F Prime data-only interpretation of the EPP Packet Length field. Deployments upgrading from that behavior should verify that external EPP encoders place the complete packet length, including the header, on the wire.
