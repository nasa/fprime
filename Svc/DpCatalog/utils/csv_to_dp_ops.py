...
Binary Format (17 bytes per record):
...
def parse_u32(value_str: str, field_name: str) -> int:
    ...
    try:
        ...
        raise DpOperationError(f"Invalid {field_name} '{value_str}': must be an integer")
...
def create_binary_record(op_code: int, dp_id: int, t_sec: int, t_sub: int, priority: int) -> bytes:
    """
...
def convert_csv_to_binary(csv_path: Path, output_path: Path, verbose: bool = False) -> None:
    """
        ...
        for line_num, row in enumerate(reader, start=2):  # Start at 2 (header is line 1)
            try:
                ...
                if verbose:
                    ...
                    print(f"Line {line_num}: {op_name} ID={dp_id} tSec={t_sec} tSub={t_sub} Priority={priority}")
    ...
    if verbose:
        ...
        print(f"Output file size: {len(records) * 17} bytes")
