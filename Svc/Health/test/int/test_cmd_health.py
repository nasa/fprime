...
def test_send_health_command(fprime_test_api):
    ...
    time.sleep(3)        
    cmdErrors = fprime_test_api.await_telemetry(fprime_test_api.get_mnemonic('Svc.CommandDispatcher') + "." + "CommandErrors", start="NOW")
    # If no constraints are specified on the channels, the predicate will always return true    
    param_error = fprime_test_api.get_telemetry_pred(fprime_test_api.get_mnemonic('Svc.CommandDispatcher') + "." + "CommandErrors", cmdErrors)
    ...
    fprime_test_api.send_command(fprime_test_api.get_mnemonic('Svc.FileManager') + '.' + 'AppendFile', ["/tmp/1MiB.txt", "/tmp/2MiB.txt"])
    time.sleep(50)
    ...
    # Cleanup directory
    fprime_test_api.send_command(fprime_test_api.get_mnemonic('Svc.FileManager') + '.' + 'RemoveFile', ["/tmp/2MiB.txt", True] )     
