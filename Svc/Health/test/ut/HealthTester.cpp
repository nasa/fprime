...
#define MAX_HISTORY_SIZE                                                                                         \
    (Svc::HealthComponentBase::NUM_PINGSEND_OUTPUT_PORTS * Svc::HealthComponentBase::NUM_PINGSEND_OUTPUT_PORTS * \
...
HealthTester ::HealthTester() : HealthGTestBase("Tester", MAX_HISTORY_SIZE), component("Health") {
    this->connectPorts();
    this->initComponents();
}
...
void HealthTester ::from_PingSend_handler(const FwIndexType portNum, U32 key) {
    if (this->override) {
...
void HealthTester ::from_WdogStroke_handler(const FwIndexType portNum, U32 code) {
...
void HealthTester ::connectPorts() {
    // PingReturn
    for (FwIndexType i = 0; i < this->component.getNum_PingReturn_InputPorts(); ++i) {
        this->connect_to_PingReturn(i, this->component.get_PingReturn_InputPort(i));
    }
    ...
    for (FwIndexType i = 0; i < this->component.getNum_PingSend_OutputPorts(); ++i) {
        this->component.set_PingSend_OutputPort(i, this->get_from_PingSend(i));
    }

    // WdogStroke
    this->component.set_WdogStroke_OutputPort(0, this->get_from_WdogStroke(0));
...
void HealthTester ::initComponents() {
    this->init();
...
void HealthTester ::faultTlm() {
    TEST_CASE(900.1.3, "Fault Telemetry");
    REQUIREMENT("ISF-HTH-003");
    COMMENT(
        "The Svc::Health component shall issue a FATAL event if a component fails to return a ping by the specified "
        "timeout.");
    ...
    for (U32 i = 0; i < Svc::HealthComponentBase::NUM_PINGSEND_OUTPUT_PORTS; i++) {
        this->invoke_to_Run(0, 0);
        for (FwIndexType port = 0; port < Svc::HealthComponentBase::NUM_PINGSEND_OUTPUT_PORTS; port++) {
            ASSERT_EQ(Svc::HealthComponentBase::NUM_PINGSEND_OUTPUT_PORTS * 2 + i + 1,
                      this->component.m_pingTrackerEntries[port].cycleCount);
        }
    }
    this->invoke_to_Run(0, 0);
...
void HealthTester ::disableAllMonitoring() {
    ...
    for (U32 i = 0; i < Svc::HealthComponentBase::NUM_PINGSEND_OUTPUT_PORTS; i++) {
        this->invoke_to_Run(0, 0);
        for (FwIndexType entry = 0; entry < Svc::HealthComponentBase::NUM_PINGSEND_OUTPUT_PORTS; entry++) {
            ASSERT_EQ(Svc::HealthComponentBase::NUM_PINGSEND_OUTPUT_PORTS * 2 + 1 + i,
                      this->component.m_pingTrackerEntries[entry].cycleCount);
        }
    }
    this->invoke_to_Run(0, 0);
...
void HealthTester ::disableOneMonitoring() {
...
void HealthTester ::updatePingTimeout() {
    ...
    for (FwIndexType entry = 0; entry < Svc::HealthComponentBase::NUM_PINGSEND_OUTPUT_PORTS; entry++) {
        ...
        ASSERT_EQ(this->pingEntries[entry].fatalCycles,
                  Svc::HealthComponentBase::NUM_PINGSEND_OUTPUT_PORTS * 2 + entry + 1);
        char name[80];
        snprintf(name, sizeof(name), "task%d", entry);
        Fw::CmdStringArg task(name);
        this->sendCmd_HLTH_CHNG_PING(0, 10, task, Svc::HealthComponentBase::NUM_PINGSEND_OUTPUT_PORTS * 4 + entry,
                                     Svc::HealthComponentBase::NUM_PINGSEND_OUTPUT_PORTS * 5 + entry);
        this->dispatchAll();
        // verify EVR
        ASSERT_EVENTS_SIZE(1);
        ASSERT_EVENTS_HLTH_PING_UPDATED_SIZE(1);
        ASSERT_EVENTS_HLTH_PING_UPDATED(0, name, Svc::HealthComponentBase::NUM_PINGSEND_OUTPUT_PORTS * 4 + entry,
                                        Svc::HealthComponentBase::NUM_PINGSEND_OUTPUT_PORTS * 5 + entry);
...
void HealthTester ::watchdogCheck() {
    TEST_CASE(900.1.7, "Watchdog check");
    REQUIREMENT("ISF-HTH-007");
    COMMENT(
        "The Svc::Health component shall stroke a watchdog port while all ping replies are within their limit and "
        "health checks pass");
...
void HealthTester ::nominal2CmdsDuringTlm() {
...
void HealthTester ::miscellaneous() {
    ...
    Fw::Enabled badValue;
    badValue.setSerializeValue(Fw::Enabled::NUM_CONSTANTS);
