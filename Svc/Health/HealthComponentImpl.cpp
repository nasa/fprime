...
HealthImpl::HealthImpl(const char* const compName)
    ...
    for (NATIVE_UINT_TYPE entry = 0;
         entry < FW_NUM_ARRAY_ELEMENTS(this->m_pingTrackerEntries); entry++) {
        this->m_pingTrackerEntries[entry].enabled = HLTH_PING_DISABLED;
    }
}

void HealthImpl::init(const NATIVE_INT_TYPE queueDepth,
                      const NATIVE_INT_TYPE instance) {
    HealthComponentBase::init(queueDepth, instance);
    this->queue_depth = queueDepth;
}

void HealthImpl::setPingEntries(PingEntry* pingEntries,
    ...
    for (NATIVE_INT_TYPE entry = 0; entry < numPingEntries; entry++) {
        ...
        this->m_pingTrackerEntries[entry].pingFeedback = false;
    }
}
...
void HealthImpl::PingReturn_handler(const NATIVE_INT_TYPE portNum, U32 key) {
    ...
    if (key != this->m_pingTrackerEntries[portNum].key) {
        Fw::LogStringArg _arg =
            this->m_pingTrackerEntries[portNum].entry.entryName;
        ...
        // send event feeback if current entry required one
        if (this->m_pingTrackerEntries[portNum].pingFeedback == true) {
            Fw::LogStringArg _arg =
                this->m_pingTrackerEntries[portNum].entry.entryName;
            this->log_ACTIVITY_HI_HLTH_PING_RESPONSE(_arg);
            this->m_pingTrackerEntries[portNum].pingFeedback = false;
        }
    }
}

void HealthImpl::Run_handler(const NATIVE_INT_TYPE portNum,
    ...
    if (this->m_enabled) {
        ...
        for (NATIVE_UINT_TYPE entry = 0; entry < this->m_numPingEntries;
             entry++) {
            if (HLTH_PING_ENABLED ==
                this->m_pingTrackerEntries[entry].enabled) {
                // If clear entry
                if (0 == this->m_pingTrackerEntries[entry].cycleCount) {
                    // start a ping
                    this->m_pingTrackerEntries[entry].key = this->m_key;
                    // send ping
                    this->PingSend_out(entry,
                                       this->m_pingTrackerEntries[entry].key);
                    // increment key
                    this->m_key++;
...
void HealthImpl::HLTH_ENABLE_cmdHandler(const FwOpcodeType opCode, U32 cmdSeq,
...
void HealthImpl::HLTH_PING_ENABLE_cmdHandler(const FwOpcodeType opCode,
    ...
    // check enable value
    if (enable != HealthImpl::HLTH_PING_DISABLED &&
        enable != HealthImpl::HLTH_PING_ENABLED) {
        this->cmdResponse_out(opCode, cmdSeq, Fw::COMMAND_VALIDATION_ERROR);
        return;
    }
...
void HealthImpl::HLTH_CHNG_PING_cmdHandler(const FwOpcodeType opCode,
                                           U32 cmdSeq,
                                           const Fw::CmdStringArg& entry,
                                           U32 warningValue, U32 fatalValue) {
    // check to see if entry is in range
    NATIVE_INT_TYPE entryIndex = this->findEntry(entry);
    if (-1 == entryIndex) {
        this->cmdResponse_out(opCode, cmdSeq, Fw::COMMAND_VALIDATION_ERROR);
    ...
    if (warningValue > fatalValue) {
        Fw::LogStringArg arg;
        arg = entry;
        this->log_WARNING_HI_HLTH_PING_INVALID_VALUES(arg, warningValue,
                                                      fatalValue);
        this->cmdResponse_out(opCode, cmdSeq, Fw::COMMAND_VALIDATION_ERROR);
        return;
    }
...
NATIVE_INT_TYPE HealthImpl::findEntry(Fw::CmdStringArg entry) {
    ...
    for (NATIVE_UINT_TYPE tableEntry = 0;
         tableEntry < NUM_PINGSEND_OUTPUT_PORTS; tableEntry++) {
...
void HealthImpl::HLTH_PING_cmdHandler(const FwOpcodeType opCode,
                                      const U32 cmdSeq,
                                      const Fw::CmdStringArg& entry) {
    // check to see if entry is in range
    NATIVE_INT_TYPE entryIndex = this->findEntry(entry);
    ...
    if (HLTH_PING_ENABLED == this->m_pingTrackerEntries[entryIndex].enabled) {
        // next ping triggered by run input will send feedback event
        this->m_pingTrackerEntries[entryIndex].pingFeedback = true;
    } else {
        Fw::LogStringArg _arg = entry;
        this->log_WARNING_HI_HLTH_PING_DISABLED(_arg);
    }
