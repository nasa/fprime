...
|---|---|---|---|
| `PingSend` | `output` | `[HealthPingPorts] Svc.Ping` | Ping output port |
| `PingReturn` | `async input` | `[HealthPingPorts] Svc.Ping` | Ping return port |
| `Run` | `sync input` | `Svc.Sched` | Run port |
| `WdogStroke` | `output` | `Svc.WatchDog` | Run port |

### Commands

| Name | Kind | Description |
|---|---|---|
| `HLTH_ENABLE` | `async` | A command to enable or disable health checks |
| `HLTH_PING_ENABLE` | `async` | Ignore a particular ping entry |
| `HLTH_CHNG_PING` | `async` | Change ping value |

### Events

| Name | Severity | Description |
|---|---|---|
| `HLTH_PING_WARN` | `warning high` | Warn that a ping target is longer than the warning value |
| `HLTH_PING_LATE` | `fatal` | Declare FATAL since task is no longer responding |
| `HLTH_PING_WRONG_KEY` | `fatal` | Declare FATAL since task is no longer responding |
| `HLTH_CHECK_ENABLE` | `activity high` | Report checking turned on or off |
| `HLTH_CHECK_PING` | `activity high` | Report a particular entry on or off |
| `HLTH_CHECK_LOOKUP_ERROR` | `warning low` | Entry was not found |
| `HLTH_PING_UPDATED` | `activity high` | Report changed ping |
| `HLTH_PING_INVALID_VALUES` | `warning high` | Report changed ping |

### Telemetry

| Name | Type | Description |
|---|---|---|
| `PingLateWarnings` | `U32` | Number of overrun warnings |
