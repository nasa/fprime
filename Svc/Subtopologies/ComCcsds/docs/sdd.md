...
The standard CCSDS stack composes the `SpacePacketFraming` packet layer with the
`TmTcFraming` transfer frame layer. On downlink, COM data is queued, framed into CCSDS
Space Packets, aggregated, and framed into TM Transfer Frames. On uplink, raw bytes are
accumulated into TC Transfer Frames, deframed into Space Packets, and routed into the
flight software.
...
For the variant of this flow with an SDLS security layer inserted between the packet and
