...
On the uplink path, `ComCcsdsConfig.TcDeframer.enablePacketSpanning` (see the `ComCcsds` SDD) makes `tcDeframer` reassemble packets segmented across several TC frames before they reach `SdlsDecryption`. The reassembled packet is therefore the unit the `CcsdsSdlsDeframer` sees: the ground must apply SDLS (SA index and encryption) to the whole packet first and segment the resulting security payload afterwards, so that each TC frame data field is `Segment Header | slice of (SA index | ciphertext)`.

This is a deliberate departure from the per-frame layout of CCSDS 355.0-B-2, in which every TC frame carries its own Security Header and Trailer immediately after the Segment Header and is authenticated individually. Per-frame SDLS is not supported when spanning is enabled in this subtopology, because the SDLS layer sits above the transfer frame layer: supporting it would require decrypting each segment before reassembly, i.e. moving reassembly out of `tcDeframer` into a component downstream of `decryptor`. Projects that require standard-conformant per-frame TC SDLS must keep `TcDeframer.enablePacketSpanning = false` (one packet per frame, which is then also the per-frame security unit) and bound uplinked packets to one frame data field. With an authenticating `decryptor` (not the default `ClearTextDecryptor`), the packet-level layout gives one authentication tag per packet, so a packet spliced from segments of two different packets after frame loss (see the `Svc.Ccsds.TcDeframer` SDD) fails authentication as a whole and is dropped.
