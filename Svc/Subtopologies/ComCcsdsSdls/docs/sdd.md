...
`Svc.Ccsds.CcsdsSdlsFramer` prepends a 2-byte SA index to each aggregate and the configured encryptor may add its own per-frame overhead (`ComCcsdsSdlsConfig.EncryptorOverhead`, 0 for the clear-text default), so projects must set `ComCcsdsConfig.Aggregator.aggregationSize = Svc.Ccsds.TmDataFieldSize - Svc.Ccsds.SdlsSaIndexSize - ComCcsdsSdlsConfig.EncryptorOverhead` for the encrypted aggregate plus the SA index to fill the TM data field exactly. The aggregator idle-fills each aggregate to that size before it reaches the encryptor, so idle data is authenticated and encrypted along with the packets; the First Header Pointer is relative to the data following the SA index (the SDLS security header is not part of the TM data field per CCSDS 355.0-B). The subtopology enforces this sizing with a `static_assert` in the `sdlsFramer` instance's `configComponents` phase, so a mismatched `aggregationSize` fails to compile rather than asserting in `TmFramer::dataIn_handler` at runtime. A project selecting the AES-256-GCM pair (12-byte IV plus 16-byte MAC) sets `EncryptorOverhead = 28` in its `ComCcsdsSdlsConfig` override and `aggregationSize = Svc.Ccsds.TmDataFieldSize - Svc.Ccsds.SdlsSaIndexSize - 28` (986 for the default 1024-byte frame).
