    ...
    for (FwIndexType currentPriority = 0; currentPriority < TOTAL_PORT_COUNT; currentPriority++) {
        ...
        for (FwIndexType entryIndex = 0;
            ...
            if (currentPriority == queueConfig.entries[entryIndex].priority) {
                ...
                // index of the entry. Those lower than COM_PORT_COUNT are ComBufferQueueEntry (Fw::ComBuffer + Apid)
                // and those larger are BufferQueueEntry (Fw::Buffer + Apid).
                entry.msgSize = (entryIndex < COM_PORT_COUNT) ? sizeof(ComBufferQueueEntry) : sizeof(BufferQueueEntry);
                // Overflow checks
...
void ComQueue::comPacketQueueIn_handler(const FwIndexType portNum,
                                        Fw::ComBuffer& data,
                                        const ComCfg::Apid& packetType,
                                        U32 context) {
    (void)context;  // User context is not used by ComQueue
    ...
    ComBufferQueueEntry entry;
    entry.comBuffer = data;
    entry.apid = packetType.e;
    (void)this->enqueue(portNum, QueueType::COM_QUEUE, reinterpret_cast<const U8*>(&entry), sizeof(entry));
}

void ComQueue::bufferQueueIn_handler(const FwIndexType portNum, Fw::Buffer& fwBuffer, const ComCfg::Apid& packetType) {
    ...
    entry.apid = packetType.e;
    bool success = this->enqueue(queueNum, QueueType::BUFFER_QUEUE, reinterpret_cast<const U8*>(&entry), sizeof(entry));
    if (!success) {
...
void ComQueue::bufferQueueIn_overflowHook(FwIndexType portNum, Fw::Buffer& fwBuffer, const ComCfg::Apid& packetType) {
    (void)packetType;
    FW_ASSERT(portNum >= 0 && portNum < BUFFER_PORT_COUNT, static_cast<FwAssertArgType>(portNum));
    this->bufferReturnOut_out(portNum, fwBuffer);
}
...
bool ComQueue::enqueue(const FwIndexType queueNum, QueueType queueType, const U8* data, const FwSizeType size) {
    ...
    const FwSizeType expectedSize =
        (queueType == QueueType::COM_QUEUE) ? sizeof(ComBufferQueueEntry) : sizeof(BufferQueueEntry);
...
void ComQueue::sendComBuffer(Fw::ComBuffer& comBuffer, ComCfg::Apid::T apid, FwIndexType queueIndex) {
    ...
    // APID supplied at port invocation is propagated via FrameContext; the queue index
    // is used to identify the originating port when the buffer is returned on dataReturnIn.
...
void ComQueue::drainQueue(FwIndexType index) {
    ...
    for (FwSizeType i = 0; (i < available) && (status == Fw::FW_SERIALIZE_OK); i++) {
        if (index < COM_PORT_COUNT) {
            // Dequeueing is reading the whole persisted ComBufferQueueEntry object from the queue's storage.
            ComBufferQueueEntry entry;
            ...
            // Dequeueing is reading the whole persisted BufferQueueEntry object from the queue's storage.
            BufferQueueEntry entry;
            status = queue.dequeue(reinterpret_cast<U8*>(&entry), sizeof(entry));
...
void ComQueue::processQueue() {
    ...
    for (priorityIndex = 0; priorityIndex < TOTAL_PORT_COUNT; priorityIndex++) {
        ...
        if (entry.index < COM_PORT_COUNT) {
            // Dequeue is reading the whole persisted ComBufferQueueEntry from the queue's storage.
            FW_ASSERT(this->m_buffer_state == OWNED);
            auto dequeue_status =
