...
void Tester ::testQueueSend() {
    ...
    for (NATIVE_INT_TYPE portNum = 0; portNum < ComQueue::ComQueueComSize; portNum++) {
        invoke_to_comQueueIn(portNum, comBuffer, 0);
        emitOneAndCheck(portNum, QueueType::comQueue, comBuffer, buffer);
    }
    clearFromPortHistory();

    for (NATIVE_INT_TYPE portNum = 0; portNum < ComQueue::ComQueueBuffSize; portNum++) {
        invoke_to_buffQueueIn(portNum, buffer);
        emitOneAndCheck(portNum, QueueType::buffQueue, comBuffer, buffer);
    }
    clearFromPortHistory();
    component.cleanup();
}
...
void Tester ::testRetrySend() {
    ...
    m_retry_delta = 0;
    for (NATIVE_INT_TYPE portNum = 0; portNum < ComQueue::ComQueueBuffSize; portNum++) {
        NATIVE_INT_TYPE portCount = portNum + 1;
        invoke_to_buffQueueIn(portNum, buffer);
...
void Tester ::connectPorts() {
    // buffQueueIn
    for (NATIVE_INT_TYPE i = 0; i < ComQueue::ComQueueBuffSize; ++i) {
        this->connect_to_buffQueueIn(i, this->component.get_buffQueueIn_InputPort(i));
    }

    // comQueueIn
    for (NATIVE_INT_TYPE i = 0; i < ComQueue::ComQueueComSize; ++i) {
