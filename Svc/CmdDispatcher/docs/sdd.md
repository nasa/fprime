...
An autogenerated function on components create a public function `regCommands` that tells components to register the set of op codes that are implemented by the component. The autogenerated port is connected to the `compCmdReg` input port on `Svc::CmdDispatcher` that corresponds to the number of the `compCmdSend` port used to dispatch commands. The port handler inserts the opcode into a dispatch table (a red-black tree map keyed by opcode), mapping the opcode to the dispatch port number corresponding to the registration port number. Re-registering an already-registered opcode is accepted and emits an `OpCodeReregistered` diagnostic event.
...
`Svc::CmdDispatcherCfg::ExecuteCommandWhenSequenceTrackerTableIsFull` | Selects the behavior when a command cannot be tracked because the pending sequence command table is full. See Note #5 in section 3.2.2. Default: `false`.
...
9/20/2026 | Added `ExecuteCommandWhenSequenceTrackerTableIsFull` configuration and `Fw::CmdResponse::DISPATCHED_UNTRACKED` added CD-007 and section 3.6
