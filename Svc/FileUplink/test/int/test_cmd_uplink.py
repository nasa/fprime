...
def test_send_uplink_command(fprime_test_api):
    ...
    """
    # file_path = /tmp/test_seq.seq (staged by conftest) and destination = /tmp/test_seq.seq (for fileManager)
    fprime_test_api.uplink_file_and_await_completion(
        "/tmp/test_seq.seq", "/tmp/test_seq.seq", timeout=100
    )
    # for fileDownlink
    fprime_test_api.uplink_file_and_await_completion(
        "/tmp/test_seq_wait.seq", "/tmp/test_seq_wait.seq", timeout=100
    )
    # for health, fileDownlink, fileManager
    fprime_test_api.uplink_file_and_await_completion(
        "/tmp/1MiB.txt", "/tmp/1MiB.txt", timeout=100
    )
