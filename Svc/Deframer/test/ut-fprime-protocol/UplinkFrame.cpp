...
DeframerTester::UplinkFrame::UplinkFrame(Fw::ComPacket::ComPacketType a_packetType, U32 a_packetSize)
    ...
    if ((packetType == Fw::ComPacket::FW_PACKET_COMMAND) && (packetSize <= getMaxValidCommandPacketSize())) {
        valid = true;
    }
    if ((packetType == Fw::ComPacket::FW_PACKET_FILE) && (packetSize <= getMaxValidFilePacketSize())) {
        valid = true;
    }
}
...
U32 DeframerTester::UplinkFrame::getRemainingCopySize() const {
    ...
    FW_ASSERT(frameSize >= copyOffset, static_cast<FwAssertArgType>(frameSize),
              static_cast<FwAssertArgType>(copyOffset));
...
const DeframerTester::UplinkFrame::FrameData& DeframerTester::UplinkFrame::getData() const {
    return data;
}

void DeframerTester::UplinkFrame::copyDataOut(Fw::SerialBuffer& serialBuffer, U32 size) {
    ASSERT_LE(copyOffset + size, getSize());
    const Fw::SerializeStatus status = serialBuffer.pushBytes(&data[copyOffset], size);
...
DeframerTester::UplinkFrame DeframerTester::UplinkFrame::random() {
    ...
    Fw::ComPacket::ComPacketType packetType = Fw::ComPacket::FW_PACKET_UNKNOWN;
    ...
    if (invalidSizeIndex == 0) {
        ...
        packetSize = STest::Pick::lowerUpper(getMinPacketSize(), maxValidPacketSize);
    }
    // Construct the frame
...
U32 DeframerTester::UplinkFrame::getInvalidPacketSize() {
    FW_ASSERT(MAX_FRAME_SIZE >= NON_PACKET_SIZE, MAX_FRAME_SIZE, NON_PACKET_SIZE);
    const U32 result = MAX_FRAME_SIZE - NON_PACKET_SIZE;
    // Make sure the size is invalid
    FW_ASSERT(result > getMaxValidCommandPacketSize(), static_cast<FwAssertArgType>(result),
              static_cast<FwAssertArgType>(getMaxValidCommandPacketSize()));
    FW_ASSERT(result > getMaxValidFilePacketSize(), static_cast<FwAssertArgType>(result),
              static_cast<FwAssertArgType>(getMaxValidFilePacketSize()));
    return result;
}
...
U32 DeframerTester::UplinkFrame::getMaxValidFilePacketSize() {
    FW_ASSERT(MAX_VALID_FRAME_SIZE >= NON_PACKET_SIZE, MAX_VALID_FRAME_SIZE, NON_PACKET_SIZE);
    ...
    if (valid) {
        ...
        switch (invalidateIndex) {
            case 0: {
                ...
                const FpFrameHeader::TokenType badStartWord = FpFrameHeader::START_WORD + 1;
...
void DeframerTester::UplinkFrame::writePacketSize(FpFrameHeader::TokenType ps) {
    Fw::SerialBuffer sb(&data[PACKET_SIZE_OFFSET], sizeof ps);
    const Fw::SerializeStatus status = sb.serialize(packetSize);
    ASSERT_EQ(status, Fw::FW_SERIALIZE_OK);
}

void DeframerTester::UplinkFrame::writePacketType(FwPacketDescriptorType pt) {
    Fw::SerialBuffer sb(&data[PACKET_TYPE_OFFSET], sizeof pt);
    const Fw::SerializeStatus status = sb.serialize(pt);
    ASSERT_EQ(status, Fw::FW_SERIALIZE_OK);
}

void DeframerTester::UplinkFrame::writeStartWord(FpFrameHeader::TokenType sw) {
