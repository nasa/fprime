...
def test_send_version_command(fprime_test_api):
    ...
    for count, value in enumerate(["PROJECT", "FRAMEWORK", "LIBRARY","CUSTOM","ALL"], 1):

          pred = predicates.greater_than(0)
          
          ActLo_events = fprime_test_api.get_event_pred(severity=EventSeverity.ACTIVITY_LO)
...
#            events=events,
          ...
          if count == 1:
            evr_ver = fprime_test_api.await_telemetry(fprime_test_api.get_mnemonic('Svc.Version') + "." + "ProjectVersion", start="NOW")
          elif count == 2: 
            evr_ver = fprime_test_api.await_telemetry(fprime_test_api.get_mnemonic('Svc.Version') + "." + "FrameworkVersion", start="NOW")
          ...
          if count == 1 or count == 2 :
            ...
            if evr_ver_value == report_ver_value:
              print ("COMPARE EVR vs. TELEMETRY channel and version cmd_option PASS ", evr_ver_value, report_ver_value, value)
            else:
              print ("COMPARE EVR vs. TELEMETRY channel and version cmd_option FAIL ", evr_ver_value, report_ver_value, value)

    
    fprime_test_api.send_and_assert_command(fprime_test_api.get_mnemonic('Svc.Version') + '.' + 'ENABLE',["ENABLED"], max_delay=1)

    fprime_test_api.send_and_assert_command(fprime_test_api.get_mnemonic('Svc.Version') + '.' + 'ENABLE',["DISABLED"], max_delay=1)                
