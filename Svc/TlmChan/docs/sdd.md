...
The `Run` handler swaps the active buffer under the component mutex and, still under the mutex, drains the newly active buffer's set element by element, clearing each recorded bucket's flag as it goes. The set is not cleared wholesale, because `Fw::RedBlackTreeSet::clear()` rebuilds the set's free list at a cost proportional to `TLMCHAN_HASH_BUCKETS`. After releasing the mutex, the handler walks the inactive buffer's set, in ascending bucket order, to serialize and send the updated channels. Both steps therefore cost time proportional to the number of channels updated in a cycle rather than to `TLMCHAN_HASH_BUCKETS` (the locked drain carries a logarithmic factor per removal). The mutex is held only for the swap and the drain; the drain's size is the number of buckets updated in the cycle before last, including any entries deferred by the processing cap, and is independent of the send pass.
...
In order to speed up lookups for storing and reading telemetry channels, a seeded hash function is used to select a location in an array of hash table slots. Configuration values in `TlmChanImplCfg.hpp` define the number of hash buckets and slots. The number of buckets must be at least as large as the number of telemetry channels defined in the system. Each bucket also reserves a node in each buffer's updated set, about 112 bytes per bucket across the two sets on a 64-bit target (roughly 56 KB at the default of 500), in addition to the bucket entry itself; budget memory accordingly when sizing `TLMCHAN_HASH_BUCKETS`. The number of channels in the system can be determined by invoking `make comp_report_gen` from the deployment directory. `TLMCHAN_NUM_TLM_HASH_SLOTS` and `TLMCHAN_HASH_MOD_VALUE` can be varied to balance the amount of memory for slots versus the distribution of buckets to slots. See `TlmChanImplCfg.hpp` for tuning guidance.
