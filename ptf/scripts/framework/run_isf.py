...
parser.add_option("-t",default="isf_linux_helper", dest="target", help="Set target environment.")
...
parser.add_option("-a",default="isf_gse",dest="gds",help="Set gse program.")
parser.add_option("-d",default="Ref", dest="deployment",help="Specify a deployment")
...
args = "target=" + options.target + " compiler=" + options.compiler + " gds=" + options.gds

# Setup deployment here
os.environ['DEPLOYMENT'] = options.deployment
...
# This will always be interactive...
...
for arg in sys.argv:
	if arg != sys.argv[0]:
		if arg.find(".sh") != -1 or arg.find(".py") != -1:
			...
			test_list.append((os.path.splitext(os.path.basename(arg))[0],os.path.abspath(arg),None))
		...
		if arg.find(".suite") != -1:
			...
			for test in suite_test_list:
				...
				if (len(test)):
					...
					test_list.append((desc,os.path.abspath(script),env_list))
...
for test in test_list:
	...
	# LJR hack to make this wait before completing
	#quit = False
	#while quit == False:
	#	quit = raw_input("Running 'push q to quit': ")
	#	if quit == 'q':
	#		quit = True
	#	else:
	#		quit = False
	...
	if status == None:
		outcome = "killed"
		print("%-60.60s%30.30s"%(testdesc,"KILLED"))
		summary_file.write("%-60.60s%30.30s\n"%(testdesc,"KILLED"))
		a_test_failed = True;
		break
	elif status == 0:
		outcome = "passed"
		print("%-60.60s%30.30s"%(testdesc,"PASSED"))
		summary_file.write("%-60.60s%30.30s\n"%(testdesc,"PASSED"))
	else:
		outcome = "failed"
