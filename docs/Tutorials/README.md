...
This will include looking through existing portions of the `Ref` app in order to learn the tools and termonologies of an
F´ project.
...
components communicate and run and F´ project without the need for embedded devices nor hardware. This tutorial runs entirly
on a user's *nix (Linux, Mac, BSD, Windows w/ WSL) machine.
...
a Linux VM, and have a small UART/USB GPS receicer.
