...
Go to the directory `Ref` and run `mkdir MathTypes`.
This directory will contain our new type.

In the directory `Ref` at the top level of the F Prime repository,
...
Create a file `MathTypes/CMakeLists.txt`
...
Now we need to add the new directory to the project.
