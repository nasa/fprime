...
- **Too Many Pending Commands** — If the pending command tracker is full, a `TooManyCommands` warning event is logged. By default (`Svc::CmdDispatcherCfg::ExecuteCommandWhenSequenceTrackerTableIsFull = false`) the command is rejected with `EXECUTION_ERROR`. When the setting is `true`, the command is dispatched anyway and the source receives `DISPATCHED_UNTRACKED`, meaning its completion status will never be reported.
