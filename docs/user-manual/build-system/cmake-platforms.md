...
`CHOOSES_IMPLEMENTATIONS`, and `GLOBAL_IMPLICIT_DEPENDENCY` directives.
...
`GLOBAL_IMPLICIT_DEPENDENCY`: links the platform configuration into the global interface target so every F´ module receives it
without an explicit `DEPENDS` entry. Replaces the deprecated `BASE_CONFIG` flag.
