...
3. Implement `<autocoder name>_setup_autocode AC_INPUT_FILE)` to run the aurocoder on files filter by item 2.
...
`<autocoder name>_is_supported` takes one argument: a source file path to a possible autocoder input. If the autoocder
...
```
executable [--arg [[--arg]] intput1 [input2...]
```
...
function(touch_autocode_setup AC_INPUT_FILE)
    ...
    # Only need to supply and monitor the single input file, no configuration, or auxilary files
