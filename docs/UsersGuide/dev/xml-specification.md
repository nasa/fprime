...
| arg                        | pass\_by  | Optional. Specifies how the argument should be passed to a handler function. Values can be POINTER or REFERENCE. For a synchronous port handler, the default behavior is to pass by value for primitive values, otherwise by const reference. POINTER causes a pointer to be passed. REFERENCE causes a mutable reference to be passed. For an asynchronous port handler, POINTER causes a pointer to the data to be passed. Otherwise the data itself is serialized, copied into and out of the queue, and deserialized. Use of pointers and mutable references carries the usual responsibility for understanding the scope and lifetime of the memory behind the pointer, as the pointer value may be held by another component past the duration of the port call. |
