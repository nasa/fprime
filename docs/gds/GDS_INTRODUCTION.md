...
While most users can use the default GDS parameters for a while, there are times when you'll want to change certain GDS options. Here're a few particularly common cases when that happens, along with a list of all the options you have when running the GDS.
...
Below is a full list of options you can use when running the `fprime-gds` command to start the GDS. You can view this menu at any time by running `fprime-gds --help`.
