// ======================================================================
// \title  LinuxGpioDriverV2.cpp
// \brief  cpp file for LinuxGpioDriverV2 component implementation class
//
// \copyright
...
static Os::File::Status errno_to_file_status(int errno_input) {
...
static Drv::GpioStatus errno_to_gpio_status(int errno_input) {
...
static U64 configuration_to_line_flags(Drv::LinuxGpioDriverV2::GpioConfiguration configuration) {
    U64 flags = 0;
    switch (configuration) {
        case LinuxGpioDriverV2::GPIO_OUTPUT:
            flags = GPIO_V2_LINE_FLAG_OUTPUT;
            break;
        case LinuxGpioDriverV2::GPIO_INPUT:
            flags = GPIO_V2_LINE_FLAG_INPUT;
            break;
        case LinuxGpioDriverV2::GPIO_INTERRUPT_RISING_EDGE:
            flags = GPIO_V2_LINE_FLAG_INPUT | GPIO_V2_LINE_FLAG_EDGE_RISING;
            break;
        case LinuxGpioDriverV2::GPIO_INTERRUPT_FALLING_EDGE:
            flags = GPIO_V2_LINE_FLAG_INPUT | GPIO_V2_LINE_FLAG_EDGE_FALLING;
            break;
        case LinuxGpioDriverV2::GPIO_INTERRUPT_BOTH_RISING_AND_FALLING_EDGES:
...
Os::File::Status LinuxGpioDriverV2 ::setupLineRequest(const int chip_descriptor,
    ...
    request.num_lines = 1;
    request.fd = -1;
    request.config.flags = configuration_to_line_flags(configuration);
...
Os::File::Status LinuxGpioDriverV2 ::open(const char* device,
    ...
    struct gpio_v2_line_info pin_info;
    (void)::memset(&pin_info, 0, sizeof pin_info);
    pin_info.offset = gpio;
    return_value = ioctl(chip_descriptor, GPIO_V2_GET_LINEINFO_IOCTL, &pin_info);
    if (return_value == 0) {
    ...
    int pin_fd = -1;
    status = this->setupLineRequest(chip_descriptor, gpio, configuration, default_state, pin_fd);
    // Final status check
