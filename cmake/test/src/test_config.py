...
def test_fprime_model_override(CONFIG_BUILD):
    ...
    cmake.assert_process_success(
        CONFIG_BUILD, warnings_ok=True, targets=["TestModelOverride"]
    )
...
def test_fprime_header_override(CONFIG_BUILD):
    """Test that the config override works"""
    cmake.assert_process_success(
        CONFIG_BUILD, warnings_ok=True, targets=["TestHeaderOverride"]
    )
...
def test_fprime_library_override(CONFIG_BUILD):
    ...
    cmake.assert_process_success(
        CONFIG_BUILD, warnings_ok=True, targets=["TestFPrimeLibraryOverride"]
    )
...
def test_library_override(CONFIG_BUILD):
    ...
    cmake.assert_process_success(
        CONFIG_BUILD, warnings_ok=True, targets=["library_config"]
    )
...
def test_library_new_config(CONFIG_BUILD):
    ...
    cmake.assert_process_success(
        CONFIG_BUILD, warnings_ok=True, targets=["TestLibraryNewConfig"]
    )
