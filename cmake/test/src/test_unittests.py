...
def test_unittest_module_ut_info(UT_BUILD):
    ...
    headers, sources, generated, ac_sources, dependencies = [line.strip().split(";") for line in lines]
    assert ["SignalGenTester.hpp"] == [Path(header).name for header in headers], "Did not find expected headers"
    assert sorted(["SignalGenTester.cpp", "SignalGenTestMain.cpp"]) == sorted([Path(source).name for source in sources]), "Did not find expected sources"
    expected_ac = ["SignalGen.fpp", "Commands.fppi", "Events.fppi", "Telemetry.fppi"]
    actual_ac = [Path(source).name for source in ac_sources]
    assert sorted(expected_ac) == sorted(actual_ac), "Did not find expected autocoder sources"
    expected_gen = ['SignalGenComponentAi.xml', 'SignalInfoSerializableAi.xml', 'SignalPairSerializableAi.xml', 'SignalPairSetArrayAi.xml', 'SignalSetArrayAi.xml', 'SignalTypeEnumAi.xml', 'SignalGenComponentAc.cpp', 'SignalGenComponentAc.hpp', 'SignalInfoSerializableAc.cpp', 'SignalInfoSerializableAc.hpp', 'SignalPairSerializableAc.cpp', 'SignalPairSerializableAc.hpp', 'SignalPairSetArrayAc.cpp', 'SignalPairSetArrayAc.hpp', 'SignalSetArrayAc.cpp', 'SignalSetArrayAc.hpp', 'SignalTypeEnumAc.cpp', 'SignalTypeEnumAc.hpp',
                    "SignalGenGTestBase.cpp","SignalGenGTestBase.hpp", "SignalGenTesterBase.cpp", "SignalGenTesterBase.hpp", "SignalGenTesterHelpers.cpp"]
    actual_gen = [Path(source).name for source in generated]
    assert sorted(expected_gen) == sorted(actual_gen), "Did not find expected autocoder generated sources"
