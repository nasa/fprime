...
function(add_deployment_target MODULE TARGET SOURCES DIRECT_DEPENDENCIES FULL_DEPENDENCY_LIST)
    ...
    add_module_target("${MODULE}" "${TARGET}" "${SPOURCES}")
...
####
function(add_module_target MODULE TARGET SOURCES DEPENDENCIES_DEFUNCT)
    if (NOT DEFINED FPRIME_PRESCAN)
        return()
    endif()
    ...
    foreach(SOURCE IN LISTS SOURCES)
        is_supported("${SOURCE}")
        if (IS_SUPPORTED)
            file(APPEND "${FPP_LIST_FILE}" ";${SOURCE}")
            list(APPEND TARGET_FPPS "${SOURCE}")
        endif()
    endforeach()
    if (TARGET_FPPS)
