...
from fprime_python_model.semantics.component_instance import ComponentInstance as fpm_ComponentInstance
...
from fprime_python_model.semantics.interface_instance import InterfaceComponentInstance as fpm_InterfaceComponentInstance
...
class PriorityBufferAnalyzer:
    ...
    def __init__(self, build_dir: Path, topology_path: Path, output_file: Path, verbose: bool = False):
        ...
        for comp_instance in component_instances:
            try:
                ...
                if result:
                    ...
                    logger.info(f"  ✓ {result.component_path}: priorities {sorted(result.priority_buffers.keys())}")
            except (FileNotFoundError, ValueError, KeyError, AttributeError) as e:
                logger.info(f"  ✗ Failed to analyze {comp_instance.get_qualified_name()}: {e}")
                if self.verbose:
                    traceback.print_exc()
                fail_count += 1
    ...
    def get_component_instances(self, topology: fpm_Topology) -> List[fpm_ComponentInstance]:
        ...
        for interface_instance, location in topology.instance_map.items():
            if isinstance(interface_instance, fpm_InterfaceComponentInstance):
                ...
                logger.debug(f"  Component instance: {comp_instance.get_qualified_name()}")
        
        return component_instances
    
    def _load_component_from_model(self, comp_model: fpm_FprimePythonModel, component_dir: Path) -> fpm_Component:
        ...
        if len(comp_ids) != 1:
            raise ValueError(f"Multiple component definitions in single component file: {comp_ids} in {component_dir}")

        return comp_analysis.component_map[comp_ids[0]]

    def _should_skip_component(self, priority_buffers: Dict[int, List[PortBufferInfo]]) -> bool:
    ...
    def _load_and_analyze_component(self, component_dir: Path) -> Optional[Dict[int, List[PortBufferInfo]]]:
    ...
    def _create_component_info(self, component: fpm_Component, component_dir: Path,
                               priority_buffers: Dict[int, List[PortBufferInfo]]) -> ComponentPriorityInfo:
        ...
        self.component_headers.add(f"{relative_path}/{component_name.base}ComponentAc.hpp")
    ...
    def analyze_component_instance(self, comp_instance: fpm_ComponentInstance) -> Optional[ComponentPriorityInfo]:
    ...
    def find_component_build_dir(self, comp_instance: fpm_ComponentInstance, component: fpm_Component) -> Optional[Path]:
    ...
    def load_component_model(self, component_dir: Path) -> Optional[fpm_FprimePythonModel]:
        """Load component model using fpm_FprimePythonModel"""
        model = self._load_fprime_model(component_dir)
        if not model:
            logger.debug(f"  Missing JSON files or failed to load model from {component_dir}")
        return model
    
    def extract_priority_buffers(self, component: fpm_Component, analysis: fpm_Analysis) -> Dict[int, List[PortBufferInfo]]:
    ...
    def extract_async_ports(self, component: fpm_Component, analysis: fpm_Analysis, priority_map: Dict[int, List[PortBufferInfo]]) -> None:
        ...
        for port_name, port_instance in component.port_map.items():
            if isinstance(port_instance, fpm_port_instance.GeneralPortInstance):
                if port_instance.kind == fpm_fpp_ast.fpp_ast.GeneralKind.ASYNC_INPUT:
                    ...
                    buffer_expr = self.get_port_buffer_size_expr(port_instance, analysis)
                    ...
                    if buffer_expr:
                        ...
                        priority_map[priority].append(PortBufferInfo(
                            port_name=str(port_name),
                            port_type=port_type,
                            buffer_expr=buffer_expr,
                            is_internal=False
                        ))
                        logger.debug(f"  Async port '{port_name}' at priority {priority}: {buffer_expr}")
            
            elif isinstance(port_instance, fpm_port_instance.InternalPortInstance):
                priority = self.get_port_priority(port_instance, analysis)
                buffer_expr = self.calculate_internal_port_buffer(port_instance, analysis)
                ...
                if buffer_expr:
                    ...
                    logger.debug(f"  Internal port '{port_name}' at priority {priority}: {buffer_expr}")
    
    def extract_async_commands(self, component: fpm_Component, analysis: fpm_Analysis, priority_map: Dict[int, List[PortBufferInfo]]) -> None:
        ...
        for opcode, command in component.command_map.items():
            if isinstance(command, fpm_command.CommandNonParam):
                if isinstance(command.kind, fpm_command.NonParamKindAsync):
                    ...
                    logger.debug(f"  Async command '{command.get_name()}' at priority {priority}")
        ...
        for priority in async_command_priorities:
            ...
            has_cmd = any(p.buffer_expr ==  self.get_qual_port_buffer_size_expr(port_type) for p in priority_map[priority])
    ...
    def get_port_priority(self, port_instance: fpm_port_instance.PortInstance, analysis: fpm_Analysis) -> int:
        ...
        if isinstance(port_instance, fpm_port_instance.GeneralPortInstance):
            if port_instance.specifier.priority is not None:  
                ...
                # Handle named constant (e.g., "ActiveRateGroupOutputPorts")
                    ...
                    priority_val = analysis.value_map[port_instance.specifier.priority._id].value
                else:
    ...
    def get_command_priority(self, command: fpm_command.CommandNonParam, analysis: fpm_Analysis) -> int:
    ...
    def get_port_type_name(self, port_instance: fpm_port_instance.GeneralPortInstance, analysis: fpm_Analysis) -> str:
    ...
    def get_port_buffer_size_expr(self, port_instance: fpm_port_instance.GeneralPortInstance, analysis: fpm_Analysis) -> str:
    ...
    def _is_primitive_type(self, resolved_type: fpm_types_values.Type) -> bool:
        ...
        return isinstance(resolved_type, (
    ...
    def resolved_type_to_cpp_size_expr(self, resolved_type: fpm_types_values.Type, analysis: fpm_Analysis) -> str:
        ...
        """
        match resolved_type:
            case fpm_types_values.PrimitiveIntType():
                return f"sizeof({resolved_type.kind.name})"
            case fpm_types_values.FloatType():
                return f"sizeof({resolved_type.kind.name})"
            case fpm_types_values.BooleanType():
                return "sizeof(FwEnumStoreType)"
            case fpm_types_values.StringType():
            ...
            case fpm_types_values.AliasType():
            ...
            case fpm_types_values.AbsType() | fpm_types_values.StructType() | fpm_types_values.ArrayType() | fpm_types_values.EnumType():
                symbol = resolved_type.get_def_symbol()
                qn = analysis.get_qualified_name_from_map(symbol)
                cpp_name = "::".join(qn.to_ident_list())
    ...
    def calculate_internal_port_buffer(self, port_instance: fpm_port_instance.InternalPortInstance, analysis: fpm_Analysis) -> str:
        ...
        try:
            ...
            for annotated_param in params:
                ...
                logger.debug(f"    param: {formal_param.name}, type: {formal_param.type_name}")
                ...
                type_size_expr = self.resolved_type_to_cpp_size_expr(resolved_type, analysis)
                logger.debug(f"    type_size_expr: {type_size_expr!r}, resolved_type: {resolved_type!r}")
                size_expressions.append(type_size_expr)

            return ' + '.join(size_expressions)

        except (AttributeError, KeyError) as e:
            logger.warning(f"Warning: Could not calculate internal port size: {e}\n{port_instance.__dict__!r}")
            raise
    ...
    def _get_unique_buffer_exprs(self, buffers: List[PortBufferInfo]) -> Tuple[List[str], List[str]]:
    ...
    def _generate_priority_constant(self, lines: List[str], priority: int, unique_exprs: List[str], port_list: str, comp_path: str) -> None:
        ...
        if len(unique_exprs) == 0:
            raise ValueError(f"No buffer expressions found for {comp_path} priority {priority}")
    ...
    def generate_header(self, components: List[ComponentPriorityInfo]) -> None:
        ...
        lines.append("")
        lines.append("constexpr FwSizeType DATA_OFFSET = sizeof(FwEnumStoreType) + sizeof(FwIndexType);")
        lines.append("")
        ...
        for comp_info in sorted(components, key=lambda c: c.component_path):
            ...
            for priority in sorted(comp_info.priority_buffers.keys()):
                ...
                self._generate_priority_constant(lines, priority, unique_exprs, port_list, comp_info.component_path)
