...
excluded_folders = {
    ...
    "tutorials-led-blinker",
    "tutorials-arduino-led-blinker",
    "tutorials-math-component",
}
...
def build_nav_tree(path):
    ...
    for entry in entries:
        if (
            entry == "fprime"
        ):  # Entire fprime repo is copied, but only want to nav fprime/docs
            entry = "fprime/docs"
        ...
        # Process folders only if they contain .md files inside
            ...
            if (
                entry.lower() == "tutorials"
            ):  # manually defines tutorial tree since its links are external
...
def write_mkdocs_nav(root_path, output_file="mkdocs_nav.yml"):
    ...
    yaml_output = yaml.dump(
        nav_tree, sort_keys=False, default_flow_style=False, allow_unicode=True
    )
