...
namespace Os {
    const U32 SystemResources::LINUX_CPU_LINE_LIMIT = 1024; // Maximum lines to read before bailing
    ...
    SystemResources::SystemResourcesStatus SystemResources::getCpuCount(U32 &cpuCount)
    {
        ...
        for (U32 i = 0; i < SystemResources::LINUX_CPU_LINE_LIMIT; i++) {
    ...
    SystemResources::SystemResourcesStatus SystemResources::getMemUtil(MemUtil& memory_util)
    {
        ...
        // Check results
        if (total < 0 or used < 0 or total < used) {
            return SYSTEM_RESOURCES_ERROR;
        }
        mem_util.total = total * 1024; // Reported in KB
        mem_util.used = (total - free) * 1024;
        return SYSTEM_RESOURCES_OK;
    }
}
