...
struct Tester {
     ...
     */
    virtual std::shared_ptr<char> get_filename(bool random) const = 0;
     ...
     * @return: size of file
     */
    virtual FwSizeType size(const char* filename) const = 0;

    /**
     * Assert that the supplied status is appropriate for the current mode.
     * @param status: status to check
     */
    void assert_valid_mode_status(Os::File::Status& status) {
    ...
    std::map<char*, std::vector<U8>> filesystem;
