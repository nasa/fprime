...
// Static debug flag (disabled by default)
std::atomic<bool> AtomicQueue::s_debugEnabled(false);

void AtomicQueue::setDebugLogging(bool enable) {
    s_debugEnabled.store(enable, std::memory_order_relaxed);
}
...
void AtomicQueue::create(FwSizeType numBuffers, 
    ...
    for (FwSizeType i = 0; i < numBuffers; ++i) {
        ...
        FW_ASSERT(slot->sequence.is_lock_free(), static_cast<FwAssertArgType>(i), static_cast<FwAssertArgType>(numBuffers));
    }
...
void AtomicQueue::teardown() {
    ...
    if (this->m_slots != nullptr) {
        ...
        for (FwSizeType i = 0; i < this->m_capacity; ++i) {
            FW_ASSERT(i < this->m_capacity, static_cast<FwAssertArgType>(i), static_cast<FwAssertArgType>(this->m_capacity));
            this->m_slots[i].~Slot();
        }
...
bool AtomicQueue::enqueueInternal(const U8* buffer, FwSizeType size) {
    ...
    FW_ASSERT(size <= this->m_bufferSize, static_cast<FwAssertArgType>(size), static_cast<FwAssertArgType>(this->m_bufferSize));
    
    for (FwSizeType retry = 0; retry < MAX_CAS_RETRIES; ++retry) {
        FW_ASSERT(retry < MAX_CAS_RETRIES, static_cast<FwAssertArgType>(retry));
        
        // acquire-release on slot->sequence ensures coherence, enqueuePos & dequeuePos are relaxed since strong memory ordering is not required
        ...
        if (diff == 0) {
            ...
            if (this->m_enqueuePos.compare_exchange_weak(pos, pos + 1,
...
bool AtomicQueue::dequeue(U8* buffer, FwSizeType capacity, FwSizeType& actualSize) {
    ...
    for (FwSizeType retry = 0; retry < MAX_CAS_RETRIES; ++retry) {
        ...
        if (diff == 0) {
            ...
            if (this->m_dequeuePos.compare_exchange_weak(pos, pos + 1,
                                                         std::memory_order_release,
                                                         std::memory_order_relaxed)) {
...
FwSizeType AtomicQueue::getSize() const {
    ...
    FW_ASSERT(static_cast<FwSizeType>(diff) <= this->m_capacity, 
              static_cast<FwAssertArgType>(diff), static_cast<FwAssertArgType>(this->m_capacity));
