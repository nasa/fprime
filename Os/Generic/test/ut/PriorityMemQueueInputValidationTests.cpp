...
class BoundaryTest : public PriorityMemQueueInputValidation,
...
INSTANTIATE_TEST_SUITE_P(AllBoundaries, BoundaryTest, ::testing::Values(
    ValidationCase{"MinPriority", 0, 1, Os::QueueInterface::Status::OP_OK},
    ValidationCase{"MaxPriority", Os::Generic::Queue::MAX_PRIORITIES - 1, 1, Os::QueueInterface::Status::OP_OK},
    ValidationCase{"InvalidPriority", Os::Generic::Queue::MAX_PRIORITIES, 10, Os::QueueInterface::Status::INVALID_PRIORITY},
    ValidationCase{"InvalidPriorityPlus", Os::Generic::Queue::MAX_PRIORITIES + 1, 10, Os::QueueInterface::Status::INVALID_PRIORITY},
    ValidationCase{"MaxMsgSize", 0, 128, Os::QueueInterface::Status::OP_OK},
    ValidationCase{"OversizeMsgSize", 0, 129, Os::QueueInterface::Status::SIZE_MISMATCH}
));
...
TEST_F(PriorityMemQueueInputValidation, DepthAndSizeLimits) {
    struct {FwSizeType depth, msgSize;} cases[] = {{1, 128}, {100, 512}};
    
    for (auto& c : cases) {
        Os::Generic::PriorityMemQueue queue;
        ASSERT_EQ(Os::QueueInterface::Status::OP_OK, 
                  queue.create(1, Fw::String("Q"), c.depth, c.msgSize));
