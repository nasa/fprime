...
Directly run cosmosgen.py using either Python 2 or Python 3 and make sure that the environment variables PYTHON_BASE, PYTHONPATH, BUILD_ROOT and FPRIME_CORE_DIR are properly set. The COSMOS tool also uses the environment variable COSMOS_USERPATH, which must be set to the location of your COSMOS config directory i.e. fprime-sw/COSMOS.
