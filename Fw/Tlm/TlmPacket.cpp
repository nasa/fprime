...
namespace Fw {
    ...
    SerializeStatus TlmPacket::addValue(FwChanIdType id, Time& timeTag, TlmBuffer& buffer) {
        ...
        // telemetry buffer
        stat = this->m_tlmBuffer.serializeFrom(buffer.getBuffAddr(),buffer.getBuffLength(),Fw::Serialization::OMIT_LENGTH);
        if (stat != Fw::FW_SERIALIZE_OK) {
            return stat;
        }
    ...
    SerializeStatus TlmPacket::serializeTo(SerializeBufferBase& buffer) const {
        ...
        return buffer.serializeFrom(this->m_tlmBuffer.getBuffAddr(),m_tlmBuffer.getBuffLength(),Fw::Serialization::OMIT_LENGTH);
    }
