...
class AbstractMethodException(TypeException):
    def __init__(self, val):
        super(AbstractMethodException,self).__init__("%s must be implemented since it is abstract!" % str(val))
...
class StringSizeException(TypeException):
    def __init__(self, size, max_size):
        super(StringSizeException,self).__init__("String size %s is greater than %s!" % (str(size), str(max_size)) )
...
class NotInitializedException(TypeException):
    def __init__(self, message):
        super(NotInitializedException,self).__init__("Instance %s not initialized!"%message)

class NotOverridenException(TypeException):
    def __init__(self, message):
        super(NotOverridenException,self).__init__("Required base class method not overrwritten in type %s!"%message)

class ArgLengthMismatchException(TypeException):
    def __init__(self, arg_length_actual, arg_length_given):
        super(ArgLengthMismatchException,self).__init__("%d args provided, but command expects %d args!"%(arg_length_given, arg_length_actual))

class CompoundTypeLengthMismatchException(TypeException):
    def __init__(self, field_length_actual, field_length_given):
        super(CompoundTypeLengthMismatchException,self).__init__("%d fields provided, but compound type expects %d fields!"%(field_length_given, field_length_actual))
