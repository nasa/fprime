...
__CHEETAH_genTime__ = 1558020935.5685031
...
class Config_System(Template):
    ...
    def respond(self, trans=None):
        ...
        write('''# Created on ''')
        _v = VFFSL(SL,"user",True) # '${user}' on line 1, col 14
        if _v is not None: write(_filter(_v, rawExpr='${user}')) # from line 1, col 14.
        write('''
# Author: ''')
        _v = VFFSL(SL,"date",True) # '${date}' on line 2, col 11
        if _v is not None: write(_filter(_v, rawExpr='${date}')) # from line 2, col 11.
        write('''
''')
        write('''# THIS FILE IS AUTOMATICALLY GENERATED - ONLY ADD NEW TARGET DEFINITIONS
''')
        write('''# System Documentation: http://cosmosrb.com/docs/system/
# Declare Targets that make up the system
''')
        _v = '\n'
        if _v is not None: write(_filter(_v))
        write('''# DECLARE_TARGET target_name [substitute_name]
''')
        for name in VFFSL(SL,"names",True): # generated from line 10, col 1
            write('''DECLARE_TARGET ''')
            _v = VFFSL(SL,"name",True) # '$name' on line 11, col 16
            if _v is not None: write(_filter(_v, rawExpr='$name')) # from line 11, col 16.
            write('''
''')
        write("""
...
if not hasattr(Config_System, '_initCheetahAttributes'):
    templateAPIClass = getattr(Config_System,
