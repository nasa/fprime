...
#            PRINT.info("WARNING: Interface %s renamed to %s."  % (name2,name_str))
    ...
    def msgTypedefName(self, id, name, name_sep='AcMsg'):
        ...
        if len(id_list) == 1:
            ...
            name_list = [id_list[0][0].upper() + id_list[0][1:]] + \
    ...
    def msgTokenName(self, id, name):
        ...
        if len(id_list) == 1:
            ...
            name_list = [id_list[0].upper()] + [id_list[1].upper()] + ["AC_MSG"] + name_list[2:]
        new_name = '_'
        new_name = new_name.join(name_list)
        return(new_name)
...
#### AND MAKE IT SIMPLE.
    ...
    def formatFunCommentOldVersion(self, name, args, proto=True, indent=0):
        ...
        if self.commentInArgsPresent(args) == True:
            ...
            func_arg_string = ""
            format_func = (format_func_name + func_arg_string.join(func_arg_list)).strip('\n')
        return format_func
    ...
    def argTypedefStringAlign(self, args):
        ...
        str = self.argStringAlign(type_list, arg_list, int(self.__config.get('ipc','type_arg_spaces')))
...
#        str = self.argStringAlign(str, comment_list, int(self.__config.get('ipc','arg_comment_spaces')))
    ...
    def subThreadInstanceFirstArg(self, mod_id, args, context_list):
        ...
        if self.subThreadTest(mod_id) == True and len(context_list) > 0:
            ...
            arg_comment = "This argument specifies which %s subtask will receive the command." % (mod_id)
            id = ("id",instance_enum,arg_comment,"","",[],["16", instance_enum, file])
            args = [id] + args
    ...
    def argFilter(self, module, args, name="foo"):
        ...
        new_args = list()
        for arg in args:
            id, type, comment, array_max, array_len, range_list, enum_type_list = arg[0:7]

            if type == "STRING":
                id2 = id + "_size"
                array_len = id2
                new_args.append((id, "U8", comment, array_max, array_len, range_list, enum_type_list))

                comment = "Size of string argument %s" % id
                new_args.append((id2, "U16", comment, "", "", [], []))

            elif type == "SCLK":
                type = module[0].upper() + module[1:] + "ArgSclk"
                type = "TimSpacecraftTime"
                new_args.append((id, type, comment, array_max, array_len, range_list, enum_type_list))
