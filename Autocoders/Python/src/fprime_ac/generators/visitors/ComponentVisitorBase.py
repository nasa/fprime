...
class ComponentVisitorBase(AbstractVisitor.AbstractVisitor):
    ...
    def buildFileName(self, obj):
        ...
        if self.config("component","XMLDefaultFileName") == "True":
            filename = obj.get_namespace() + obj.get_name() + self.config("component", self.__visitor)
            DEBUG.info("Generating code filename: %s, using XML namespace and name attributes..." % filename)
        else:
            ...
            if (x[0][-l:] == s[0]) & (x[1] == s[1]):
                filename = x[0].split(s[0])[0] + self.config("component", self.__visitor)
                DEBUG.info("Generating code filename: %s..." % filename)
            else:
                msg = "XML file naming format not allowed (must be XXXComponentAi.xml), Filename: %s" % xml_file
                PRINT.info(msg)
                raise
        return filename
    ...
    def initCommands(self, obj, c):
        ...
        c.command_param_strs = self.__model_parser.getCommandArgsPrototypeStringDict(obj)
    ...
    def initInternalInterfaces(self, obj, c):
        ...
        c.internal_interface_enums = self.__model_parser.getInternalInterfaceEnumList(obj)
        c.internal_interfaces = self.__model_parser.getInternalInterfacesList(obj)
        c.internal_interface_param_strs = self.__model_parser.getInternalInterfaceArgsPrototypeStringDict(obj)
        c.internal_interface_args_str_dict = c.internal_interface_param_strs
        c.internal_interface_args = self.__model_parser.getInternalInterfaceArgsDict(obj)
        c.internal_interface_args_str = self.internalInterfaceArgsStr()
        c.internal_interface_params = self.__model_parser.getInternalInterfaceArgsDict(obj, True)
    ...
    def initInputPorts(self, c):
        ...
        c.input_ports = [
            ...
            for (instance, type, direction, sync, priority, full, role, max_num)
            in c.ports_all
            if direction == "Input"
        ]
        # User input ports
        c.user_input_ports = [
            (instance, type, sync, priority, full, role, max_num)
            for (instance, type, sync, priority, full, role, max_num)
            in c.input_ports
            if role is None
        ]
        # Serial input ports
    ...
    def initOutputPorts(self, c):
        ...
        c.user_output_ports = [
            (instance, type, sync, priority, role, max_num)
            for (instance, type, sync, priority, role, max_num)
            in c.output_ports
            if role is None
        ]
        # Typed output ports
    ...
    def initInvocationFunctions(self, c):
        ...
        c.invocation_functions = [
            (instance, type, sync, priority, role, max_num)
            for (instance, type, sync, priority, role, max_num)
            in c.output_ports if role is None
        ]
        # Invocation functions for typed output ports
        c.typed_invocation_functions = [
            (instance, type, sync, priority, role, max_num)
            for (instance, type, sync, priority, role, max_num)
            in c.typed_output_ports if role is None
        ]
    ...
    def initMessageCalls(self, c):
        ...
        c.message_calls_typed = [
            (instance, type, sync, priority, full, role, max_num)
            for (instance, type, sync, priority, full, role, max_num)
            in c.typed_input_ports if role != "Cmd"
        ]
    ...
    def isPrimitiveType(self, type):
      return type in ["I8", "U8", "I16", "U16", "I32", "U32", "I64", "U64", "F32", "F64", "bool"]
