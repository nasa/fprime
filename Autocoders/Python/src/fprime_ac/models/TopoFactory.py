...
class TopoFactory:
    ...
    def __init__(self):
        ...
        self.__table_info.append(["REQUESTED WINDOW SIZE" , 0 , "Specified by either the 'base_id_range' attribute in the instance tag or by finding the max of the 'base_id_range' attribute in the topology tag and the largest internal ID of the instance."])
        self.__table_info.append(["DIFFERENCED ID WINDOW SIZE" , 0 , "Calculated by subtracting the current base ID from the next base ID."])
        self.__table_info.append(["LARGEST COMPONENT INTERNAL ID" , 0 , "The largest ID found in the events, channels, and commands of the instance."])
        self.__table_info.append(["MAX AMOUNT OF IDS" , 0 , "The largest amount of items from either events, channels, or commands."])
        ...
        for component in components:
            ...
            for connection in x.get_connections():
                if component.get_name() == connection.get_source()[0]:
                    port = Port.Port(connection.get_source()[1], connection.get_source()[2], None, None,  comment = connection.get_comment(), xml_filename = x.get_xml_filename)
    ...
    def __compute_base_ids(self, assembly_base_id, assembly_window, instances , xml_file_path):
        ...
        if assembly_base_id == None:
            ...
            PRINT.info("WARNING: No assembly base Id set, defaulting to %s" % assembly_base_id)
        ...
        while(True):
            ...
            if with_ID_obj  == None and without_ID_obj != None: #If there is nothing in the with ID list, but items exist in the without ID list
                ...
                without_ID_obj = None
            elif with_ID_obj  != None and without_ID_obj == None: #If items exist in the with  ID list but don't in the without ID list
    ...
    def __print_base_id_table(self , base_id_tuple , actual_window_size , save_buffer):
        ...
        if base_id_tuple == None:
            ...
            data_row.append(str(base_id_tuple[1]) + " (" + str(hex(base_id_tuple[1])) +")")
    ...
    def __set_base_id_list(self, id, size, inst):
        ...
        if inst.get_base_id() == None:
            ...
            if id > abs(int(inst.get_base_id())):
                ...
                PRINT.info("WARNING: {} instance adding instance supplied base ID to the topology supplied base ID (New ID is {}) because instance supplied base ID is smaller than the topology supplied base ID.".format(n ,b))
            else:
        ...
        if inst.get_base_id_window() != None:
            ...
            if size > component_calculated_window_range:
                w = size
                PRINT.info("{} instance reseting base id window range to default topology size ({})".format(n,w))
            else:
