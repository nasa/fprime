...
namespace Tlm {
...
#endif
  {
    ...
    this->tlmHistory_AQuat =
      new History<TlmEntry_AQuat>(maxHistorySize);
    // Clear history
    this->clearHistory();
  }
    ...
    )
  {
    ...
    switch (id - idBase) {
      ...
      case TestTlmComponentBase::CHANNELID_AQUAT:
      {
        Ref::Gnc::Quaternion arg;
        const Fw::SerializeStatus _status = val.deserialize(arg);
        if (_status != Fw::FW_SERIALIZE_OK) {
          printf("Error deserializing AQuat: %d\n", _status);
          return;
        }
        this->tlmInput_AQuat(timeTag, arg);
        break;
      }
  ...
  {
    TlmEntry_AQuat e = { timeTag, val };
    this->tlmHistory_AQuat->push_back(e);
    ++this->tlmSize;
  }
