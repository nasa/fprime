...
namespace Tlm {
    ...
    )
  {
    ...
    switch (id - idBase) {
      case 100:
      {
        _status = val.serialize(_testerBase->m_param_AQuat);
        _ret = _testerBase->m_param_AQuat_valid;
        FW_ASSERT(
    ...
    )
  {
    ...
    switch (id - idBase) {
      case 100:
      {
        Ref::Gnc::Quaternion AQuatVal;
        _status = val.deserialize(AQuatVal);
        FW_ASSERT(
            _status == Fw::FW_SERIALIZE_OK,
            static_cast<AssertArg>(_status)
        );
  ...
  {
    this->m_param_AQuat = val;
    this->m_param_AQuat_valid = valid;
  }
    ...
    )
  {
    ...
    _prmOpcode =  TestPrmComponentBase::OPCODE_AQUAT_SET + idBase;
...

  {
    ...
    FwOpcodeType _prmOpcode;
    const U32 idBase = this->getIdBase();
    _prmOpcode = TestPrmComponentBase::OPCODE_AQUAT_SAVE + idBase;
    if (not this->m_to_CmdDisp[0].isConnected()) {
      printf("Test Command Output port not connected!\n");
    }
    else {
