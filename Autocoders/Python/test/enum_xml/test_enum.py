...
def compare_genfile(filename):
    ...
    if not (filecmp.cmp(filename, "templates/{}".format(filename2))):
        print(
            "WARNING: {} generated incorrectly according to Autocoders/Python/test/enum_xml/templates/{}".format(
                filename, filename2
            )
        )
...
def test_enum():
    ...
    try:
        ...
        if "FPRIME_FRAMEWORK_PATH" in os.environ.keys():
            print(
                "Found FPRIME_FRAMEWORK_PATH = {}".format(
                    os.environ["FPRIME_FRAMEWORK_PATH"]
                )
            )
        else:
            assert False
        ...
        pserial.expect(
            "(?=.*Serial1SerializableAc.cpp)(?=.*Serial1SerializableAc.hpp)(?!.*ERROR).*",
            timeout=5,
        )
        ...
        pport.expect(
            "(?=.*Port1PortAc.cpp)(?=.*Port1PortAc.hpp)(?!.*ERROR).*", timeout=5
        )
        ...
        try:
            ...
            pcmake = pexpect.spawn(
                "cmake {} -DCMAKE_BUILD_TYPE=TESTING".format(os.environ["BUILD_ROOT"])
            )
            pcmake.expect(
            ...
            for i in range(0, 6):
                ...
                if tc_data[i] == 1:
                    print(
                        "Using enum1 with integer literal value {}, enum2 with value {}, serializable with arg1 value {}, serializable with arg2 value {}".format(
                            str(enum1_literal_data[i]),
                            str(enum2_literal_data[i]),
                            str(serial1_data[i]),
                            str(serial2_data[i]),
                        )
                    )
                    ...
                    print(
                        "Using enum1 with enum object value {}, enum2 with value {}, serializable with arg1 value {}, serializable with arg2 value {}".format(
                            str(enum1_data[i]),
                            str(enum2_data[i]),
                            str(serial1_data[i]),
                            str(serial2_data[i]),
                ...
                ptestrun.expect(
                    r"(?=.*Serialized enum1)(?=.*Serialized enum2)(?=.*Deserialize enums \(y\/n\):).*",
                    timeout=3,
                )
                ptestrun.sendline("y")
                ...
                ptestrun.expect(
                    r"(?=.*Deserialized enum1)(?=.*Deserialized enum2)(?=.*Run enum check \(y\/n\):).*",
                    timeout=3,
                )
                ptestrun.sendline("y")
                ...
                ptestrun.expect(
                    r"(?=.*Successful enum1 check)(?=.*Successful enum2 check)(?=.*Create serializable \(y\):).*",
                    timeout=3,
                )
                ptestrun.sendline("y")
                ...
                if tc_data[i] == 1:
                    ptestrun.expect(
                        r"(?=.*Created serializable with enum arg)(?=.*enum should be used to invoke inst1 \(1-2\))",
                        timeout=3,
                    )
                elif tc_data[i] == 2:
                    ptestrun.expect(
                ...
                if tc_data[i] == 1:
                    ptestrun.expect(
                        r"(?=.*enum should be used to invoke inst2 \(1-2\))(?=.*inst1 Invoked ExEnumIn_handler\(0, {}, {}, {}, {}\))(?=.*inst2 Invoked EnumIn_handler\(0, {}, {}, {}, {}\)).*".format(
                            enum1_literal_data[i]
                            if (i % 2) + 1 == 1
                            else enum2_literal_data[i],
                            serial1_data[i],
                            serial2_data[i],
                    ...
                    ptestrun.expect(
                        r"(?=.*enum should be used to invoke inst2 \(1-2\))(?=.*inst1 Invoked EnumIn_handler\(0, {}, {}, {}, {}\)).*".format(
                ...
                if tc_data[i] == 1:
                    ptestrun.expect(
                        r"(?=.*run or q to quit: )(?=.*inst2 Invoked ExEnumIn_handler\(0, {}, {}, {}, {}\))(?=.*inst1 Invoked EnumIn_handler\(0, {}, {}, {}, {}\)).*".format(
                            enum1_literal_data[i]
                            if ((i + 1) % 2) + 1 == 1
                            else enum2_literal_data[i],
                            serial1_data[i],
                    ...
                    ptestrun.expect(
                        r"(?=.*run or q to quit: )(?=.*inst2 Invoked EnumIn_handler\(0, {}, {}, {}, {}\)).*".format(
                            enum_dict[enum1_data[i]]
                            if ((i + 1) % 2) + 1 == 1
                            else enum_dict[enum2_data[i]],
                            serial1_data[i],
    ...
    ## IF THEY ARE CALLED, USER SHOULD CHECK IF TEST NEEDS UPDATING.
        ...
        if expect_step == "enum":
            ...
            print(
                "EOF Error. Pattern was not matched. Serial1SerializableAi.xml generated errors when run in codegen.py"
            )
        elif expect_step == "port":
            print(
                "EOF Error. Pattern was not matched. Port1PortAi.xml generated errors when run in codegen.py"
            )
        elif expect_step == "component":
            print(
                "EOF Error. Pattern was not matched. Component1ComponentAi.xml generated errors when run in codegen.py"
            )
        ...
        # These states all correspond with equivalent of FW_ASSERTS in cpp when testing serialize / deserialize / port invocation
            ...
            print(
                "EOF Error. Test did not build successfully or input was not performed correctly."
            )
            ...
            print(
                "EOF Error. enums are not what was expected after serialize/deserialize"
            )
        elif expect_step == "cmake_serializable":
            print(
                "EOF Error. main.cpp asserted when putting enum value into serializable"
            )
