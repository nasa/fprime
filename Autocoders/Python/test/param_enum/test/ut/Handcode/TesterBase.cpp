...
namespace Prm {
  ...
  void TestPrmTesterBase ::
    paramSet_enumparam(
        const TestPrmComponentBase::SomeEnum& val,
        Fw::ParamValid valid
    )
  {
    ...
    )
  {
    ...
    _prmOpcode =  TestPrmComponentBase::OPCODE_ENUMPARAM_SET + idBase;
...

  {
    ...
    FwOpcodeType _prmOpcode;
    const U32 idBase = this->getIdBase();
    _prmOpcode = TestPrmComponentBase::OPCODE_ENUMPARAM_SAVE + idBase;
    if (not this->m_to_CmdDisp[0].isConnected()) {
      printf("Test Command Output port not connected!\n");
    }
    else {
